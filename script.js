...
window.verificarMenor = function(p) {
    ...
    if (i && t === 'MENOR') { 
        i.value = "MENOR-SIN-DNI"; i.disabled = true; 
    } else if (i) { 
...
window.navegar = async function(p) {
    window.logError(`Navegando a: ${p}`);
    
    if(!currentUserData) {
        window.logError("BLOQUEO: Usuario no cargado en variable global.");
        return;
    }
    ...
    if(p === 'home') {
        ...
        window.safeShow('screen-mantenimiento');
    }
    else if(p === 'operativa') {
...
window.iniciarSesion = async function() { 
    try { 
        ...
        window.logError("Login fail: " + e.message);
        alert(e.message); 
    } 
};
window.cerrarSesion = function() { signOut(auth); location.reload(); };

// --- 4. GESTIÓN ALBERGUES (CARGA) ---
window.cargarAlberguesActivos = function() {
    window.logError("Cargando lista de albergues activos...");
    const c = window.el('lista-albergues-activos');
    if(!c) { window.logError("ERROR: No encuentro el div lista-albergues-activos"); return; }
    
    c.innerHTML = "<p>Buscando albergues...</p>";
    
    unsubscribeAlberguesActivos = onSnapshot(query(collection(db,"albergues"),where("activo","==",true)), (s) => {
        window.logError(`Albergues encontrados: ${s.size}`);
        c.innerHTML = "";
        if(s.empty) {
            c.innerHTML = "<p>No hay albergues activos.</p>";
            return;
        }
        s.forEach(d => {
            const data = d.data();
            ...
            div.innerHTML = `<h3>${data.nombre}</h3><div class="mto-info">Entrar</div>`;
...
window.cargarDatosYEntrar = async function(id) {
    window.logError(`Entrando en albergue ID: ${id}`);
    ...
    try {
        ...
        if(dS.exists()) { 
            ...
            window.logError(`Datos cargados: ${currentAlbergueData.nombre}`);
        }
        ...
        window.logError("Error al entrar: " + e.message);
...
window.conectarListenersBackground = function(id) {
    ...
    unsubscribeAlbergueDoc = onSnapshot(doc(db,"albergues",id), d=>{ 
        if(d.exists()){ currentAlbergueData=d.data(); totalCapacidad=parseInt(currentAlbergueData.capacidad||0); window.actualizarContadores(); } 
    });
    ...
    unsubscribePersonas = onSnapshot(collection(db,"albergues",id,"personas"), s=>{
        ...
        // Refresh active selection if open
        if(personaEnGestion) { 
            const u=listaPersonasCache.find(x=>x.id===personaEnGestion.id); 
            if(u) window.seleccionarPersona(u); 
        }
    });
};

// --- 5. LOGICA MAPAS Y CAMAS ---
...
window.guardarCama = async function(c) {
    ...
    try {
        ...
        window.cerrarMapaCamas(); window.seleccionarPersona(null); window.safeHide('panel-gestion-persona');
...
window.cargarAlberguesMantenimiento = function() {
    ...
    unsubscribeAlberguesMto = onSnapshot(query(collection(db,"albergues")), s => {
        ...
        s.forEach(d => {
            ...
            // FIX: Template literal quotes carefully handled
...
window.abrirModalAlbergue = async function(id=null) {
    ...
    window.safeShow('modal-albergue');
    const b = window.el('btn-delete-albergue');
    
    if(id) {
        const s = await getDoc(doc(db,"albergues",id)); 
        const d = s.data();
        window.setVal('mto-nombre', d.nombre); 
        window.setVal('mto-capacidad', d.capacidad); 
        window.setVal('mto-columnas', d.columnas);
        
        const r = (currentUserData.rol||"").toLowerCase().trim();
        if(r === 'super_admin') window.safeShow('btn-delete-albergue'); else window.safeHide('btn-delete-albergue');
    } else { 
        window.setVal('mto-nombre',""); 
        window.setVal('mto-capacidad',""); 
        window.safeHide('btn-delete-albergue'); 
    }
};
...
window.guardarSiCorresponde = async function() {
    ...
    } catch(e) { window.logError("Autosave skipped"); }
};
