...
// --- NEW V35: NON-DESTRUCTIVE AUTO-SAVE LISTENER ---
...
// --- OBSERVATORIO V35 (SPLIT VIEW + MINI MAP) ---
window.cargarObservatorio=async function(){
    const list=window.el('obs-list-container'); if(!list)return;
    const layout = window.el('obs-layout-container');
    if(layout) layout.classList.remove('active'); // Reset split
    ...
    try{
        ...
        for(const ds of sSnap.docs){
            ...
            let color = "low"; if(sP > 70) color = "med"; if(sP > 90) color = "high";
            
            // CLICK WHOLE ROW TO OPEN MAP
            h+=`<div class="obs-row" onclick="window.activarVistaSplitObservatorio('${ds.id}')">
                <div class="obs-row-title">${d.nombre}</div>
                <div style="display:flex;width:100%;justify-content:space-between;flex-wrap:wrap;">
                    <div class="obs-data-point"><span>Espera</span><strong>${sW}</strong></div>
                    <div class="obs-data-point"><span>Alojados</span><strong>${sH}</strong></div>
...
window.activarVistaSplitObservatorio = async function(albId) {
    const layout = window.el('obs-layout-container');
    const miniGrid = window.el('mini-bed-grid');
    const title = window.el('mini-map-title');
    
    // Highlight selected
    const rows = document.querySelectorAll('.obs-row');
    rows.forEach(r => r.classList.remove('selected'));
    // Find the clicked one logic is complex without ID, skipping highlight for simplicity or use event.target logic
    
    layout.classList.add('active');
    miniGrid.innerHTML = '<div class="spinner"></div>';

    try {
        const [docSnap, pSnap] = await Promise.all([
            getDoc(doc(db, "albergues", albId)),
            getDocs(query(collection(db, "albergues", albId, "personas"), where("estado", "==", "ingresado")))
        ]);
        
        const albData = docSnap.data();
        title.innerText = "Mapa: " + albData.nombre;
        const cap = parseInt(albData.capacidad || 0);
        const cols = parseInt(albData.columnas || 8);
        
        let occupiedBeds = {};
        pSnap.forEach(d => {
            const p = d.data();
            if(p.cama) occupiedBeds[p.cama] = true;
        });
        ...
        for(let i=1; i<=cap; i++) {
            ...
            d.className = "mini-bed " + (occupiedBeds[n] ? "bg-busy" : "bg-free");
            d.innerHTML = `<div class="mini-bed-content">${n}</div>`;
            miniGrid.appendChild(d);
        }

    } catch(e) {
        miniGrid.innerHTML = "Error cargando mapa.";
    }
};

window.cerrarDetalleObservatorio = function() {
    window.el('obs-layout-container').classList.remove('active');
};
