...
// LOGICA TABS (FIX V8.5 - REMOVE HIDDEN)
window.cambiarPestana = (t) => {
    ...
    if (t === 'prefiliacion') {
        ...
        p.classList.remove('hidden'); // Fix importante
        ...
        f.classList.remove('hidden'); // Fix importante
...
window.buscarEnPrefiliacion = () => {
    ...
    // FIX V8.5: SOLO MOSTRAR ESTADO ESPERA
    const hits = listaPersonasCache.filter(p => 
        p.estado === 'espera' && // FILTRO CLAVE
...
function seleccionarPersona(p){
    ...
    panel.classList.remove('hidden'); // Asegura quitar hidden
