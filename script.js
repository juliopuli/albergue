...
onAuthStateChanged(auth,async(u)=>{
    ...
    if(u){
        ...
        if(s.exists()){
            ...
            configurarDashboard();
            window.navegar('home'); // V20.0.1 logic
        }
    } else {
        document.getElementById('app-shell').classList.add('hidden');
...
function configurarDashboard(){
    ...
    const btnHeader = document.getElementById('header-btn-users');
    if(['super_admin', 'admin'].includes(r)) btnHeader.classList.remove('hidden');
...
// --- USUARIOS LOGIC (GLOBAL SCOPE FIX) ---
