...
window.sysLog("Sistema Iniciado. Versión 2.8.0 (FULL CODE)", "info");
...
// --- FUNCIONES UI (MOVIDAS AL PRINCIPIO) ---
...
// --- NOTIFICACIONES GLOBAL (SOLUCIÓN DEFINITIVA) ---
...
window.cargarObservatorio = async function() { 
    ...
    window.el('kpi-espera').innerText = "-"; 
    window.el('kpi-alojados').innerText = "-"; 
    window.el('kpi-libres').innerText = "-"; 
    window.el('kpi-percent').innerText = "-%"; 
    ...
    try { 
        ...
        window.el('kpi-espera').innerText = totalEspera; 
        window.el('kpi-alojados').innerText = totalAlojados; 
        window.el('kpi-libres').innerText = globalLibres; 
        window.el('kpi-percent').innerText = `${globalPercent}%`; 
...
window.abrirModalInfoCama = function(p) {
    ...
    if(['admin','super_admin'].includes(currentUserData.rol)) { window.safeShow('btn-historial-cama'); bh.onclick = () => window.verHistorial(p.id); } 
...
onAuthStateChanged(auth, async (u) => {
    ...
    if(u){
        window.suscribirNotificacionesGlobales(); // INICIA ESCUCHA GLOBAL
