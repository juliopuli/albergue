...
window.buscarEnPrefiliacion = () => {
    ...
    // FIX: Filtrar solo los que están en ESPERA
...
// --- RESTORED: INFOGRAFIA & OCCUPANCY (V9.0) ---
window.cargarAlberguesActivos=()=>{
    ...
    unsubscribeAlberguesActivos=onSnapshot(query(collection(db,"albergues"),where("activo","==",true)),s=>{
        ...
        s.forEach(async d=>{
            ...
            div.innerHTML=`
                <h3>${a.nombre}</h3>
                <div class="mto-info">
                    <strong>${count}</strong> / ${a.capacidad} Ocupadas
                    <div style="height:6px; background:#e2e8f0; border-radius:3px; margin-top:5px; overflow:hidden;">
                        <div style="width:${(count/a.capacidad)*100}%; background:var(--primary); height:100%;"></div>
                    </div>
                </div>`;
...
function mostrarGridCamas(){
    ...
    for(let i=1; i<=totalCapacidad; i++){
        ...
        const occupant = listaPersonasCache.find(p => p.cama === n); // Get full object
        ...
        if(esMiCama){
            ...
            classes += " bed-busy";
            label += ` <span>${occupantName.split(' ')[0]}</span>`; // First Name only
        } else {
            classes += " bed-free";
            if(shadowMap[n]){
                classes += " bed-shadow";
        ...
        if(!window.modoMapaGeneral && !occupantName && !esMiCama){
            if(assignedMembers.length > 0){
                // Suggest next to family
                if(shadowMap[n] === myFamId) classes += " bed-suggest-target";
            } else {
                // Suggest block for whole family
                let fit=true;
        ...
        d.onclick=()=>{
            if(occupantName){
                window.abrirModalInfoCama(occupant); // Show info if occupied
            } else if(!window.modoMapaGeneral){
                guardarCama(n); // Assign if free and in assignment mode
            }
        };
        g.appendChild(d);
    }
