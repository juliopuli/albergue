...
window.darSalidaPersona = async function() {
    ...
    try {
        ...
        await batch.commit(); alert("Salida realizada."); window.seleccionarPersona(null); window.safeHide('panel-gestion-persona'); window.buscarPersonaEnAlbergue();
    } catch(e) { alert("Error: " + e.message); }
};

window.abrirModalAlbergue=async function(id=null){
    albergueEdicionId=id; window.safeShow('modal-albergue'); const b=window.el('btn-delete-albergue');
    if(id){ const s=await getDoc(doc(db,"albergues",id)); const d=s.data(); window.setVal('mto-nombre',d.nombre); window.setVal('mto-capacidad',d.capacidad); window.setVal('mto-columnas',d.columnas); const r=(currentUserData.rol||"").toLowerCase().trim(); if(r==='super_admin')window.safeShow('btn-delete-albergue');else window.safeHide('btn-delete-albergue'); } 
...
window.adminPrefiliarManual=async function(silent=false){
    ...
    const n=window.safeVal('man-nombre');if(!n)return alert("Falta nombre");const fid=new Date().getTime().toString();const t=window.getDatosFormulario('man');t.estado='espera';t.familiaId=fid;t.rolFamilia='TITULAR';t.fechaRegistro=new Date();const ref=await addDoc(collection(db,"albergues",currentAlbergueId,"personas"),t);window.registrarLog(ref.id,"Registro Manual","Titular");
    if(!silent) alert("Guardado Local"); window.limpiarFormulario('man');
};
