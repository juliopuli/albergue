...
window.vincularAFamilia = async (target) => {
    ...
    // Buscamos a TODOS los que estén en la familia del objetivo
...
// --- GESTIÓN FAMILIARES (QR PÚBLICO - ISOLATED FIX V12) ---
window.abrirModalFamiliar = () => {
    // Clear and Show
    document.getElementById('fam-nombre').value = "";
    document.getElementById('fam-ap1').value = "";
    document.getElementById('fam-ap2').value = "";
...
window.guardarFamiliarEnLista = () => {
    // Manual Extraction to avoid ID conflicts
    const nombre = document.getElementById('fam-nombre').value;
    const ap1 = document.getElementById('fam-ap1').value;
    const ap2 = document.getElementById('fam-ap2').value;
    const tipoDoc = document.getElementById('fam-tipo-doc').value;
    const docNum = document.getElementById('fam-doc-num').value;
    const fecha = document.getElementById('fam-fecha').value;

    if (!nombre) return alert("Nombre obligatorio");
    if (tipoDoc !== 'MENOR' && !docNum) return alert("Documento obligatorio");

    const familiar = { nombre, ap1, ap2, tipoDoc, docNum, fechaNac: fecha };
    listaFamiliaresTemp.push(familiar);
