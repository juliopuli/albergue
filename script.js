...
window.sysLog("Sistema Iniciado. Versión 2.7.3 (Full)", "info");

// --- GLOBALES ---
let isPublicMode = false;
let currentAlbergueId = null; 
let alberguesMap = {}; // Cache de nombres
...
// --- NAVEGACIÓN Y RESET ---
window.resetIntervencion = function() { personaEnGestion = null; window.safeHide('view-scan-result'); window.safeShow('view-scan-ready'); if(typeof resetScannerUI==='function') resetScannerUI(); };
window.detenerEscaner = function() { if (html5QrCode && html5QrCode.isScanning) { html5QrCode.stop().then(() => { html5QrCode.clear(); }).catch(e=>console.log(e)).finally(() => { if(typeof resetScannerUI==='function') resetScannerUI(); }); } };
...
window.navegar = function(p) {
    ...
    // NO DESCONECTAMOS NOTIFICACIONES GLOBALES AQUÍ
...
window.cargarDatosYEntrar = async function(id) {
    ...
    try {
        ...
        // Las notificaciones globales YA están suscritas, no necesitamos suscribir aquí de nuevo
...
// --- SIGUE EN PARTE 2 ---
// --- PARTE 2 (NOTIFICACIONES GLOBALES) ---

// 1. Suscripción Global (Se llama al login)
window.suscribirNotificacionesGlobales = async function() {
    if(unsubscribeNotifs) unsubscribeNotifs();
    
    // Cachear nombres de albergues
    try {
    ...
    unsubscribeNotifs = onSnapshot(q, (snapshot) => {
        ...
        let count = 0;
        listaNotificacionesCache = []; // Reiniciar cache local
        ...
        window.sysLog(`Error Notif: ${error.message}`, "error");
    });
};

// 2. Gestionar el Clic (Contextual)
window.gestionarClicGlobo = function() {
    ...
    // CASO A: Estamos DENTRO de un albergue
    ...
    // CASO B: Estamos FUERA (Home/Resumen)
    } else {
        // Agrupar
        const resumen = {};
...
// 3. Atender (Con Retardo de Cortesía para que cargue la ficha)
window.atenderNotificacion = async function(notifId, personaId, tabDestino) {
    ...
    // RETARDO DE 200ms PARA DAR TIEMPO A QUE CARGUE LA PESTAÑA
    setTimeout(() => {
...
// --- SIGUE EN PARTE 3 ---
// --- PARTE 3 (RESTO DE FUNCIONES RESTAURADAS) ---
...
window.registrarIntervencion = async function(tipo) { if(!personaIntervencionActiva) return; const subtipo = window.safeVal(`sel-int-${tipo}`); const detalle = window.safeVal(`det-int-${tipo}`); const nombrePersona = personaIntervencionActiva.nombre; if(!subtipo) return alert("Selecciona un tipo."); try { const data = { fecha: new Date(), usuario: currentUserData.nombre, tipo: TIPOS_INTERVENCION[tipo].titulo, subtipo: subtipo, detalle: detalle }; await addDoc(collection(db, "albergues", currentAlbergueId, "personas", personaIntervencionActiva.id, "intervenciones"), data); window.showToast("Intervención Registrada"); window.sysLog(`Intervención ${tipo} registrada para ${nombrePersona}`, "success"); window.cerrarFormularioIntervencion(tipo); } catch(e) { console.error(e); alert("Error al guardar: " + e.message); } };
