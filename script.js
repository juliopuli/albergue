...
// --- NEW V37: SMART SAVE (CHECKPOINTS) ---
...
window.darSalidaPersona = async function() {
    if(!confirm("¿Confirmar SALIDA definitiva?\n\nLa persona liberará la cama y pasará al registro global para poder ser recuperada en otro albergue.")) return;
    
    await window.guardarSiCorresponde(); // Save any pending edits first

    const p = personaEnGestion;
    const batch = writeBatch(db);

    // 1. Update Local (Archive)
    const localRef = doc(db, "albergues", currentAlbergueId, "personas", p.id);
    batch.update(localRef, { cama: null, estado: 'salida', fechaSalida: new Date() });

    // 2. Add to Global Pool
    // Generate a new ID based on timestamp to avoid collision or use docNum if unique. 
    // We'll use auto-ID for safety.
    const globalRef = doc(collection(db, "personas_globales"));
    const globalData = { ...p, cama: null, estado: 'disponible_global', origenId: currentAlbergueId, fechaSalidaOrigen: new Date() };
    delete globalData.id; // Remove local ID
    batch.set(globalRef, globalData);

    // 3. Log
    const logRef = doc(collection(db, "albergues", currentAlbergueId, "personas", p.id, "historial"));
    batch.set(logRef, { fecha: new Date(), usuario: currentUserData.nombre, accion: "Salida Albergue", detalle: "Transferido a Global" });

    try {
        await batch.commit();
        alert("Salida registrada. Persona disponible en red global.");
        window.cambiarPestana('filiacion'); // Refresh view
    } catch(e) {
...
window.navegar = async function(p) {
    if(currentUserData) await window.guardarSiCorresponde(); // SAVE ON NAVIGATE
...
window.cambiarPestana = async function(t) {
    await window.guardarSiCorresponde(); // SAVE ON TAB SWITCH
...
window.buscarEnPrefiliacion=async function(){
    ...
    // 1. Local Search
    const localHits=listaPersonasCache.filter(p=>{if(p.estado!=='espera') return false;const full=`${p.nombre} ${p.ap1||''} ${p.ap2||''}`.toLowerCase();return full.includes(t)||(p.docNum||"").toLowerCase().includes(t)||(p.telefono||"").includes(t);});
    
    // 2. Global Search (Async)
    let globalHits = [];
    try {
        // Simple global search simulation: get all global pool (production should use where clauses)
        const gSnap = await getDocs(collection(db, "personas_globales"));
        gSnap.forEach(d => {
            const p = d.data(); p.id = d.id; p.isGlobal = true;
            const full=`${p.nombre} ${p.ap1||''} ${p.ap2||''}`.toLowerCase();
            if(full.includes(t)||(p.docNum||"").toLowerCase().includes(t)) globalHits.push(p);
        });
    } catch(e) { console.log("Global search error", e); }

    r.innerHTML="";
    
    // Render Local
    localHits.forEach(p=>{r.innerHTML += `<div class="search-item" onclick="window.cargarParaEdicionPref('${p.id}')"><strong>${p.nombre} ${p.ap1||''} ${p.ap2||''}</strong><br><small>📄 ${p.docNum||'-'} | 📞 ${p.telefono||'-'}</small></div>`;});
    
    // Render Global
    globalHits.forEach(p=>{r.innerHTML += `<div class="search-item" style="background:#eff6ff;" onclick="window.cargarDesdeGlobal('${p.id}')"><strong>${p.nombre} ${p.ap1||''} <span class="badge-global">GLOBAL</span></strong><br><small>Origen: ${p.origenId || '?'} | 📄 ${p.docNum||'-'}</small></div>`;});

    window.safeShow('resultados-pref');
};

// NEW: Load from Global Pool
window.cargarDesdeGlobal = async function(gid) {
    window.safeHide('resultados-pref');
    window.el('buscador-pref').value="";
    
    const snap = await getDoc(doc(db, "personas_globales", gid));
    if(!snap.exists()) return alert("No encontrado");
    const p = snap.data();
    
    // Pre-fill form as NEW entry
    prefiliacionEdicionId = null; // Important: Treat as new
    ...
    window.el('btn-save-pref').innerText="Guardar Recuperado";
    window.safeShow('btn-cancelar-edicion-pref');
    alert("Datos recuperados. Revisa y pulsa Guardar.");
}
