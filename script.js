...
window.buscarParaVincular=()=>{const txt=document.getElementById('search-vincular').value.toLowerCase().trim();const res=document.getElementById('resultados-vincular');res.innerHTML="";if(txt.length<2){res.classList.add('hidden');return;}const hits=listaPersonasCache.filter(p=>{if(p.id===window.personaEnGestion.id)return false;const fullName=`${p.nombre} ${p.ap1||''} ${p.ap2||''}`.toLowerCase();const doc=(p.docNum||"").toLowerCase();const tel=(p.telefono||"").toLowerCase();return fullName.includes(txt)||doc.includes(txt)||tel.includes(txt);});if(hits.length===0){res.innerHTML="<div class='search-item' style='color:#999;'>No hay coincidencias.</div>";}else{hits.forEach(p=>{const d=document.createElement('div');d.className='search-item';d.innerHTML=`<strong>${p.nombre} ${p.ap1||''} ${p.ap2||''}</strong><div style="font-size:0.8rem; color:#666;"><i class="fa-regular fa-id-card"></i> ${p.docNum||'-'} &nbsp;|&nbsp; <i class="fa-solid fa-phone"></i> ${p.telefono||'-'}</div>`;d.onclick=()=>window.vincularAFamilia(p);res.appendChild(d);});}res.classList.remove('hidden');};
...
// --- EL RESTO DE FUNCIONES IGUAL QUE V4.6.0 ---
...
// --- GLOBALIZAR FUNCIONES DE CAMA (PARA EVITAR SCOPE ISSUES) ---
