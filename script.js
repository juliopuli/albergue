...
// --- 1. DEFINICIÓN DE FUNCIONES (ESTÁNDAR) ---
...
function navegar(p) {
    ...
    else if(p==='operativa') document.getElementById('screen-operativa').classList.remove('hidden');
    ...
    // Update nav active state (simplified)
...
// Todo lo definido arriba es local. Aquí lo exponemos.
...
window.guardarCama = async (c) => { if(personaEnGestion.cama) return alert("Ya tiene cama"); await updateDoc(doc(db,"albergues",currentAlbergueId,"personas",personaEnGestion.id), {estado:'ingresado',cama:c.toString(),fechaIngreso:new Date()}); cerrarMapaCamas(); };
...
// Helper for map needed for export
