...
// --- MANTENIMIENTO ALBERGUES (FIX 0/0 & ID) ---
...
window.cargarAlberguesActivos=()=>{
    ...
    unsubscribeAlberguesActivos=onSnapshot(query(collection(db,"albergues"),where("activo","==",true)),s=>{
        ...
        s.forEach(async d=>{
            ...
            div.setAttribute('onclick', "window.entrarAlbergue('" + docId + "')");
...
window.entrarAlbergue=(id)=>{
    ...
    // 2. Get People (NO ORDERBY to fix index error)
...
function mostrarGridCamas(){
    ...
    g.innerHTML="";
    if(!totalCapacidad) return; // Wait for data
    ...
    for(let i=1; i<=totalCapacidad; i++){
        ...
        let classes="bed-box";
        let label = n;
        if(esMiCama){ classes += " bed-current"; label += " (Tú)"; }
        else if(occupantName){ classes += " bed-busy"; label += ` <span>${occupantName.split(' ')[0]}</span>`; }
        else { classes += " bed-free"; if(shadowMap[n]){ classes += " bed-shadow"; } }
        ...
        d.innerHTML = label;
        d.onclick=()=>{
            if(occupantName){ window.abrirModalInfoCama(occupant); }
