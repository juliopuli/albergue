...
window.sysLog("Sistema Iniciado. Versión 1.5.2 (Secure Config)", "info");
...
// --- INIT (REORDENADO Y CORREGIDO PARA MODO PÚBLICO) ---
window.onload = async () => {
    // 1. Detectar Modo Público
    if(isPublicMode){
        window.safeHide('login-screen');
        window.safeShow('public-register-screen');
        window.safeShow('public-welcome-screen');
        window.safeHide('public-form-container');
        window.safeHide('app-shell'); // Asegurar que la app normal no se ve
        
        // 2. Iniciar sesión anónima SILENCIOSA para poder leer DB
        try {
            await signInAnonymously(auth);
            // 3. Ahora sí, leer datos del albergue
