...
// --- FUNCIONES GLOBALES DEFINIDAS ANTES DEL USO ---
...
window.abrirModalInfoCama = (p) => {
    ...
    // Botón Historial
    const bh = document.getElementById('btn-historial-cama');
...
window.guardarCama = async function(c) {
    if(window.personaEnGestion.cama){ alert("Error: Ya tiene cama."); return; }
    ...
    window.registrarLog(window.personaEnGestion.id, "Asignación Cama", `Cama ${c}`);
...
window.mostrarGridCamas = function() {
    ...
    for(let i=1; i<=totalCapacidad; i++) {
        ...
        if(!window.modoMapaGeneral && window.personaEnGestion && window.personaEnGestion.cama === n) {
            ...
            cls += " bed-busy";
            if(occ) {
                const f = `${occ.nombre} ${occ.ap1||''} ${occ.ap2||''}`;
        ...
        if(!window.modoMapaGeneral && !occName && !(!window.modoMapaGeneral && window.personaEnGestion && window.personaEnGestion.cama === n)) {
            if(assignedMembers.length > 0) {
                if(shadowMap[n] === myFamId) cls += " bed-suggest-target";
            } else {
                let fit = true;
                for(let k=0; k<neededForMe; k++) { if(camasOcupadas[(i+k).toString()]) fit = false; }
                if(fit && neededForMe > 1) cls += " bed-suggest-block";
            }
        }

        d.className = cls; d.innerHTML = lbl;
...
window.entrarAlbergue = (id) => {
    ...
    // FIX -/- : Init variables
    totalCapacidad = 0; ocupacionActual = 0;
    document.getElementById('ocupacion-count').innerText = "0";
    document.getElementById('capacidad-total').innerText = "0";
...
window.buscarPersonaEnAlbergue=()=>{
    ...
    if(hits.length===0){ res.innerHTML=`<div class="search-item" style="color:#666">No encontrado</div>`; }
        ...
        hits.forEach(p=>{
            ...
            res.innerHTML+=`<div class="search-item" onclick="window.seleccionarPersona('${p.id}')"><div style="display:flex; justify-content:space-between; width:100%; align-items:center;"><div><strong>${p.nombre} ${p.ap1||''} ${p.ap2||''}</strong><div style="font-size:0.8rem; color:#666;">📄 ${p.docNum||'Sin Doc'} | 📞 ${p.telefono||'-'}</div></div><div class="status-dot ${dc}" title="${p.estado.toUpperCase()}"></div></div></div>`;
        });
    }
    res.classList.remove('hidden');
};
