...
let prefiliacionEdicionId = null; // ID para editar en pre-filiación
...
window.cambiarPestana = (t) => {
    ...
    if (t === 'prefiliacion') {
        ...
        const p = document.getElementById('tab-prefiliacion'); p.style.display='block'; setTimeout(()=>p.classList.add('active'),10);
        ...
        // Reset Search Pref
        document.getElementById('buscador-pref').value = ""; document.getElementById('resultados-pref').style.display='none';
        window.cancelarEdicionPref();
        
    } else if (t === 'filiacion') {
        document.getElementById('btn-tab-fil').classList.add('active');
        const f = document.getElementById('tab-filiacion'); f.style.display='block'; setTimeout(()=>f.classList.add('active'),10);
        document.getElementById('buscador-persona').value = ""; document.getElementById('resultados-busqueda').style.display = 'none';
        document.getElementById('panel-gestion-persona').classList.add('hidden'); // Ocultar al cambiar
    }
};

// --- PREFILIACIÓN BUSCADOR & GUARDADO (V8.4) ---
window.buscarEnPrefiliacion = () => {
    ...
    const hits = listaPersonasCache.filter(p => (p.nombre+" "+(p.ap1||"")+" "+(p.ap2||"")+" "+(p.docNum||"")+" "+(p.telefono||"")).toLowerCase().includes(txt));
    if (hits.length === 0) { res.innerHTML = "<div class='search-item' style='color:#999;'>Sin resultados</div>"; res.style.display='block'; }
    else {
...
window.cargarParaEdicionPref = (p) => {
    ...
    // Disable family editing in quick update (to maintain integrity, editing complex families is better in Filiacion)
...
window.buscarPersonaEnAlbergue=()=>{
    ...
    if(h.length>0){r.style.display='block';h.forEach(p=>{const d=document.createElement('div');d.className='search-item';const isOut=p.estado==='espera';const badge=isOut?`<span class="badge badge-archived" style="float:right;">ESPERA</span>`:`<span class="badge badge-active" style="float:right;">DENTRO</span>`;d.innerHTML=`<div style="display:flex; justify-content:space-between; align-items:center;"><div><strong>${p.nombre} ${p.ap1||''}</strong><div class="search-details">📄 ${p.docNum||'-'}</div></div>${badge}</div>`;d.onclick=()=>{seleccionarPersona(p);r.style.display='none';document.getElementById('buscador-persona').value="";};r.appendChild(d);});}else r.style.display='none';
};
...
window.abrirModalFamiliar=()=>{limpiarFormulario('fam');document.getElementById('modal-add-familiar').classList.remove('hidden');document.getElementById('fam-tipo-doc').value="MENOR";verificarMenor('fam');};window.cerrarModalFamiliar=()=>document.getElementById('modal-add-familiar').classList.add('hidden');window.guardarFamiliarEnLista=()=>{if(document.getElementById('fam-tipo-doc').value!=='MENOR'){if(!window.validarDocumento('fam'))return alert("Documento inválido.");}const d=getDatosFormulario('fam');if(!d.nombre)return alert("Nombre obligatorio");listaFamiliaresTemp.push(d);actualizarListaFamiliaresUI();cerrarModalFamiliar();};function actualizarListaFamiliaresUI(){const d=document.getElementById('lista-familiares-ui');d.innerHTML="";if(listaFamiliaresTemp.length===0){d.innerHTML='<p style="color:#999;font-style:italic;">Ninguno añadido.</p>';return;}listaFamiliaresTemp.forEach((f,i)=>{d.innerHTML+=`<div class="fam-item"><div><strong>${f.nombre}</strong></div><button class="danger" style="margin:0;padding:2px 8px;width:auto;" onclick="borrarFamiliarTemp(${i})">X</button></div>`;});}window.borrarFamiliarTemp=(i)=>{listaFamiliaresTemp.splice(i,1);actualizarListaFamiliaresUI();};
window.abrirModalFamiliarAdmin=()=>{limpiarFormulario('adm-fam');document.getElementById('modal-admin-add-familiar').classList.remove('hidden');document.getElementById('adm-fam-tipo-doc').value="MENOR";verificarMenor('adm-fam');};window.cerrarModalFamiliarAdmin=()=>document.getElementById('modal-admin-add-familiar').classList.add('hidden');window.guardarFamiliarAdmin=()=>{if(document.getElementById('adm-fam-tipo-doc').value!=='MENOR'){if(!window.validarDocumento('adm-fam'))return alert("Datos inválidos");}const d=getDatosFormulario('adm-fam');if(!d.nombre)return alert("Nombre obligatorio");adminFamiliaresTemp.push(d);actualizarListaFamiliaresAdminUI();cerrarModalFamiliarAdmin();};function actualizarListaFamiliaresAdminUI(){const d=document.getElementById('admin-lista-familiares-ui');d.innerHTML="";if(adminFamiliaresTemp.length===0){d.innerHTML='<p style="color:#999;font-style:italic;">Ninguno.</p>';return;}adminFamiliaresTemp.forEach((f,i)=>{d.innerHTML+=`<div class="fam-item"><div><strong>${f.nombre}</strong> <small>(${f.docNum})</small></div><button class="danger" style="margin:0;padding:2px 8px;width:auto;" onclick="borrarFamiliarAdminTemp(${i})">X</button></div>`;});}window.borrarFamiliarAdminTemp=(i)=>{adminFamiliaresTemp.splice(i,1);actualizarListaFamiliaresAdminUI();};
window.publicoGuardarTodo=async()=>{if(!window.validarDocumento('pub'))return alert("Revise titular");const titular=getDatosFormulario('pub');if(!titular.nombre)return alert("Falta nombre titular");try{const famId=new Date().getTime().toString();await addDoc(collection(db,"albergues",currentAlbergueId,"personas"),{...titular,estado:'espera',fechaRegistro:new Date(),origen:'qr',familiaId:famId,rolFamilia:'TITULAR'});if(listaFamiliaresTemp.length>0){for(const f of listaFamiliaresTemp){await addDoc(collection(db,"albergues",currentAlbergueId,"personas"),{...f,estado:'espera',fechaRegistro:new Date(),origen:'qr',familiaId:famId,rolFamilia:'MIEMBRO'});}}document.getElementById('public-form-container').classList.add('hidden');document.getElementById('public-success-msg').classList.remove('hidden');listaFamiliaresTemp=[];actualizarListaFamiliaresUI();}catch(e){alert("Error: "+e.message);}};
