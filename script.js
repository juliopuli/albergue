...
if (urlParams.get('public_id')) {
    ...
    if(window.log) window.log("Modo público activado. ID: " + currentAlbergueId);
}
...
// --- FUNCIONES DEFINIDAS DIRECTAMENTE EN WINDOW (SOLUCIÓN DEFINITIVA) ---
...
window.cargarUsuarios=function(){const c=document.getElementById('lista-usuarios-container');unsubscribeUsers=onSnapshot(query(collection(db,"usuarios")),s=>{c.innerHTML="";s.forEach(d=>{const u=d.data();c.innerHTML+=`<div class="user-card-item" onclick="window.abrirModalUsuario('${d.id}')"><strong>${u.nombre}</strong><br><small>${u.rol}</small></div>`;});});};
