...
window.iniciarSesion = async () => {
    ...
    catch(e) { alert("Error: " + e.message); window.log("Login Error: " + e.message, 'error'); }
};
...
async function initPublicMode() {
    ...
    } catch(e) { window.log("Error init public: " + e.message, 'error'); }
}
...
window.verHistorial = async (pId = null, altAlbId = null) => {
    ...
    } catch (e) { content.innerHTML = "Error cargando historial."; window.log(e.message, 'error'); }
};
...
window.cargarDatosYEntrar = async (id) => {
    ...
    try {
        ...
        window.log("Error load data: " + e.message, 'error');
        document.getElementById('loading-overlay').classList.add('hidden');
    }
};
...
window.cargarObservatorio = async () => {
    ...
    } catch(e) { listContainer.innerHTML = `<p style="color:red;">Error: ${e.message}</p>`; window.log(e.message, 'error'); }
};
...
// --- HELPERS (Utility function must be defined before use) ---
