...
window.abrirModalAlbergue=async(id=null)=>{albergueEdicionId=id;document.getElementById('modal-albergue').classList.remove('hidden');if(id){const s=await getDoc(doc(db,"albergues",id));const d=s.data();safeSet('mto-nombre',d.nombre);safeSet('mto-capacidad',d.capacidad);safeSet('mto-columnas',d.columnas||8);}else{safeSet('mto-nombre',"");safeSet('mto-capacidad',"");}};
...
window.cargarAlberguesActivos=()=>{const c=document.getElementById('lista-albergues-activos');unsubscribeAlberguesActivos=onSnapshot(query(collection(db,"albergues"),where("activo","==",true)),s=>{c.innerHTML="";s.forEach(async d=>{const a=d.data();if(a.oculto&&currentUserData.rol!=='super_admin')return;const qOccupied=query(collection(db,"albergues",d.id,"personas"),where("estado","==","ingresado"));const snapOccupied=await getDocs(qOccupied);const count=snapOccupied.size;const div=document.createElement('div');const docId=d.id;div.className="mto-card";div.style.cursor="pointer";div.setAttribute('onclick',"window.entrarAlbergue('"+docId+"')");div.innerHTML=`<h3>${a.nombre}</h3><div class="mto-info"><strong>${count}</strong> / ${a.capacidad} Ocupadas<div style="height:6px; background:#e2e8f0; border-radius:3px; margin-top:5px; overflow:hidden;"><div style="width:${(count/a.capacidad)*100}%; background:var(--primary); height:100%;"></div></div></div>`;c.appendChild(div);});});};
