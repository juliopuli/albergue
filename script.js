...
window.sysLog("Sistema Iniciado. Versión 38.12.1 (Fix Mantenimiento)", "info");
...
// --- GESTIÓN ALBERGUES (ACTIVOS & MANTENIMIENTO) ---
...
// --- FIX: MANTENIMIENTO MOVIDO AQUÍ PARA EVITAR ERRORES DE ORDEN ---
...
// --- GESTIÓN DE USUARIOS (CON SEGURIDAD SUPER_ADMIN) ---
...
window.cambiarEstadoUsuarioDirecto = async function(uid, nuevoEstado) {
    ...
    if(targetDoc.exists()) {
        ...
        if(u.rol === 'super_admin') {
            ...
            window.cargarUsuarios(); // Revert visual state
            return;
        }
    }
...
window.abrirModalUsuario=async function(id=null){
    ...
    window.el('new-user-active').disabled=false; // Reset
    ...
    if(id){
        ...
        if(s.exists()){
            ...
            // Disable toggle in modal if editing super_admin
            if(d.rol === 'super_admin') {
                window.el('new-user-active').disabled = true;
            }
...
window.guardarUsuario=async function(){
    ...
    if(r === 'super_admin' && !isActive) {
        alert("Seguridad: Los Super Admin no pueden desactivarse. Se guardará como Activo.");
        isActive = true;
    }
...
// --- DESACTIVAR USUARIOS MASIVO (CON SEGURIDAD) ---
