...
window.buscarPersonaEnAlbergue = async function() {
    ...
    if(localHits.length === 0 && globalHits.length === 0){
        ...
        globalHits.forEach(p => {
            // Encode object to pass to function safely (simplified approach: use ID)
            // Storing global match in a temp cache to retrieve by ID
...
window.seleccionarPersona = function(pid) {
    ...
    window.safeShow('btn-vincular-familia'); window.safeShow('btn-historial-ficha');
    
    const r=(currentUserData.rol||"").toLowerCase().trim();
    if(!['admin','super_admin'].includes(r)) window.safeHide('btn-historial-ficha');
...
window.seleccionarPersonaGlobal = function(gid) {
    ...
    // Hide Local Actions (Cannot exit/edit until admitted)
...
window.guardarCama = async function(c) {
    ...
    // 2. GLOBAL TRANSFER & ASSIGN (ATOMIC)
    ...
    try {
        ...
        // C. Log 2: Bed
        const logRef2 = doc(collection(db, "albergues", currentAlbergueId, "personas", localRef.id, "historial")); // Auto-ID handled by calling logic usually, but here manually creating distinct ref
        // Actually, let's use a slight delay or distinct ID. Firestore batch set needs unique ref.
        // Simplified: We skip the second log in batch or create a second ref.
        // Let's create second ref:
        const logRefBed = doc(collection(db, "albergues", currentAlbergueId, "personas", localRef.id, "historial"));
        batch.set(logRefBed, { fecha: new Date(), usuario: currentUserData.nombre, accion: "Asignación Cama", detalle: `Cama ${c}` });
...
// ... (Resto de funciones: darSalida, log, etc. sin cambios importantes respecto a V39) ...
...
// Keep core functions like loading users, history, maintenance, auth, init... same as V39.
...
window.buscarEnPrefiliacion=async function(){const t=window.safeVal('buscador-pref').toLowerCase().trim();const r=window.el('resultados-pref');if(t.length<2){window.safeHide('resultados-pref');return;}const localHits=listaPersonasCache.filter(p=>{if(p.estado!=='espera')return false;const full=`${p.nombre} ${p.ap1||''} ${p.ap2||''}`.toLowerCase();return full.includes(t)||(p.docNum||"").toLowerCase().includes(t)||(p.telefono||"").includes(t);});let globalHits=[];try{const snap=await getDocs(collection(db,"pool_prefiliacion"));snap.forEach(doc=>{const p=doc.data();p.id=doc.id;p.isGlobal=true;const full=`${p.nombre} ${p.ap1||''} ${p.ap2||''}`.toLowerCase();if(full.includes(t)||(p.docNum||"").toLowerCase().includes(t))globalHits.push(p);});}catch(e){console.log(e);}r.innerHTML="";localHits.forEach(p=>{r.innerHTML+=`<div class="search-item" onclick="window.cargarParaEdicionPref('${p.id}')"><strong>${p.nombre} ${p.ap1||''} ${p.ap2||''}</strong><br><small>📄 ${p.docNum||'-'} | 📞 ${p.telefono||'-'}</small></div>`;});globalHits.forEach(p=>{r.innerHTML+=`<div class="search-item" style="background:#eff6ff;" onclick="window.cargarDesdePool('${p.id}')"><strong>${p.nombre} ${p.ap1||''} <span class="badge-global">NUBE</span></strong><br><small>📄 ${p.docNum||'-'}</small></div>`;});window.safeShow('resultados-pref');};
window.cargarParaEdicionPref=function(pid){const p=listaPersonasCache.find(x=>x.id===pid);if(!p)return;prefiliacionEdicionId=p.id;window.safeHide('resultados-pref');window.el('buscador-pref').value="";window.setVal('man-nombre',p.nombre);window.setVal('man-ap1',p.ap1);window.setVal('man-ap2',p.ap2);window.setVal('man-tipo-doc',p.tipoDoc);window.setVal('man-doc-num',p.docNum);window.setVal('man-fecha',p.fechaNac);window.setVal('man-tel',p.telefono);const l=window.el('existing-family-list-ui');l.innerHTML="";if(p.familiaId){const fs=listaPersonasCache.filter(x=>x.familiaId===p.familiaId&&x.id!==p.id);if(fs.length>0){l.innerHTML="<h5>Familiares:</h5>";fs.forEach(f=>{l.innerHTML+=`<div class="fam-item existing"><div><strong>${f.nombre} ${f.ap1||''}</strong><br><small style="color:#666;">${f.docNum||'-'} | ${f.telefono||'-'}</small></div></div>`;});}}if(['admin','super_admin'].includes(currentUserData.rol)){window.safeShow('btn-historial-pref');window.el('btn-historial-pref').onclick=()=>window.verHistorial(p.id);}else window.safeHide('btn-historial-pref');window.el('btn-save-pref').innerText="Actualizar Registro";window.safeShow('btn-cancelar-edicion-pref');window.setupAutoSave();};
window.cargarDesdePool=async function(pid){window.safeHide('resultados-pref');window.el('buscador-pref').value="";const snap=await getDoc(doc(db,"pool_prefiliacion",pid));if(!snap.exists())return;const p=snap.data();prefiliacionEdicionId=pid;window.setVal('man-nombre',p.nombre);window.setVal('man-ap1',p.ap1);window.setVal('man-ap2',p.ap2);window.setVal('man-tipo-doc',p.tipoDoc);window.setVal('man-doc-num',p.docNum);window.setVal('man-fecha',p.fechaNac);window.setVal('man-tel',p.telefono);window.safeShow('btn-ingresar-global');window.el('btn-save-pref').innerText="Guardar Nuevo en Nube";window.safeShow('btn-cancelar-edicion-pref');};
window.ingresarDesdeGlobalAction=async function(){if(!prefiliacionEdicionId)return;if(!confirm(`¿Ingresar a ${window.safeVal('man-nombre')} en este albergue?`))return;try{const batch=writeBatch(db);const data=window.getDatosFormulario('man');const newLocalRef=doc(collection(db,"albergues",currentAlbergueId,"personas"));batch.set(newLocalRef,{...data,estado:'espera',fechaIngresoAlbergue:new Date(),origenGlobal:true});const logRef=doc(collection(db,"albergues",currentAlbergueId,"personas",newLocalRef.id,"historial"));batch.set(logRef,{fecha:new Date(),usuario:currentUserData.nombre,accion:`Entrada a Albergue ${currentAlbergueData.nombre}`,detalle:"Transferido desde Nube"});const poolRef=doc(db,"pool_prefiliacion",prefiliacionEdicionId);batch.delete(poolRef);await batch.commit();alert("Persona ingresada correctamente.");window.cancelarEdicionPref();window.cambiarPestana('filiacion');}catch(e){alert("Error al ingresar: "+e.message);}};
window.crearNuevoEnPool=async function(){const data=window.getDatosFormulario('man');if(!data.nombre)return alert("Nombre obligatorio");try{await addDoc(collection(db,"pool_prefiliacion"),{...data,fechaRegistro:new Date(),estado:'espera',familiaId:new Date().getTime().toString(),rolFamilia:'TITULAR'});alert("Creado en la Nube Global");window.cancelarEdicionPref();}catch(e){alert(e.message);}};
