...
// --- 2. FUNCIONES DE UTILIDAD (DECLARADAS) ---
...
function buscarParaVincular() {
    const t = safeVal('search-vincular').toLowerCase().trim(); const r = document.getElementById('resultados-vincular'); r.innerHTML = "";
...
async function vincularAFamilia(target) {
    if (!confirm(`¿Unir a ${personaEnGestion.nombre} con la familia de ${target.nombre}?`)) return;
...
function configurarDashboard() {
    ...
    if (r === 'super_admin') { const c = document.getElementById('container-ver-ocultos'); if(c) c.classList.remove('hidden'); }
}
...
async function cargarDatosYEntrar(id) {
    ...
    try {
        ...
        qS.forEach(d => {
            const p = d.data(); p.id = d.id; listaPersonasCache.push(p);
            if (p.estado === 'ingresado') { c++; if (p.cama) camasOcupadas[p.cama] = p.nombre; }
        });
...
function cargarAlberguesActivos() {
    ...
    unsubscribeAlberguesActivos = onSnapshot(query(collection(db, "albergues"), where("activo", "==", true)), s => {
        ...
        s.forEach(async d => {
            ...
            c.appendChild(div);
            getDocs(query(collection(db, "albergues", d.id, "personas"), where("estado", "==", "ingresado"))).then(snap => {
                const el = document.getElementById(`info-${d.id}`); if (el) el.innerHTML = `Ocupación: <strong>${snap.size}</strong> / ${a.capacidad}`;
            });
        });
    });
}
...
async function registrarLog(personaId, accion, detalle = "") {
    try {
        const usuarioLog = currentUserData ? currentUserData.nombre : "Auto-Registro QR";
        await addDoc(collection(db, "albergues", currentAlbergueId, "personas", personaId, "historial"), { fecha: new Date(), usuario: usuarioLog, accion: accion, detalle: detalle });
    } catch (e) { console.error(e); }
}

async function verHistorial(pId = null, altAlbId = null) {
    ...
    try {
        ...
        let html = "";
        snap.forEach(doc => {
            const d = doc.data(); const f = d.fecha.toDate();
...
async function verListaObservatorio(albId, est) {
    ...
    try {
        ...
        if (est === 'ingresado') { dataArray.sort((a, b) => (parseInt(a.cama) || 0) - (parseInt(b.cama) || 0)); }
        else { dataArray.sort((a, b) => (b.fechaRegistro?.seconds || 0) - (a.fechaRegistro?.seconds || 0)); }
...
function buscarEnPrefiliacion() {
    ...
    r.innerHTML = "";
    hits.forEach(p => { r.innerHTML += `<div class="search-item" onclick="window.cargarParaEdicionPref('${p.id}')"><strong>${p.nombre} ${p.ap1 || ''} ${p.ap2 || ''}</strong><br><small>📄 ${p.docNum || '-'} | 📞 ${p.telefono || '-'}</small></div>`; });
    r.classList.remove('hidden');
}
...
function cargarParaEdicionPref(pid) {
    ...
    if (p.familiaId) {
        ...
        if (fs.length > 0) {
            ...
            fs.forEach(f => { l.innerHTML += `<div class="fam-item existing"><div><strong>${f.nombre} ${f.ap1 || ''}</strong><br><small>${f.docNum || '-'} | ${f.telefono || '-'}</small></div></div>`; });
        }
    }
    const btnH = document.getElementById('btn-historial-pref');
    if (['admin', 'super_admin'].includes(currentUserData.rol)) { btnH.classList.remove('hidden'); btnH.onclick = () => verHistorial(p.id); }
...
function buscarPersonaEnAlbergue() {
    ...
    const hits = listaPersonasCache.filter(p => { const full = `${p.nombre} ${p.ap1 || ''} ${p.ap2 || ''}`.toLowerCase(); return full.includes(txt) || (p.docNum || "").toLowerCase().includes(txt); });
    res.innerHTML = "";
...
function seleccionarPersona(pid) {
...
async function guardarCambiosPersona() { if (!personaEnGestion) return; const p = getDatosFormulario('edit'); await updateDoc(doc(db, "albergues", currentAlbergueId, "personas", personaEnGestion.id), p); registrarLog(personaEnGestion.id, "Edición Datos", "Manual"); alert("Guardado"); }
async function adminPrefiliarManual() {
    if (prefiliacionEdicionId) {
        const p = getDatosFormulario('man'); await updateDoc(doc(db, "albergues", currentAlbergueId, "personas", prefiliacionEdicionId), p);
        registrarLog(prefiliacionEdicionId, "Edición Pre-filiación", "Manual");
        if (adminFamiliaresTemp.length > 0) {
            const tit = listaPersonasCache.find(x => x.id === prefiliacionEdicionId); const fid = tit.familiaId || new Date().getTime().toString();
            if (!tit.familiaId) { await updateDoc(doc(db, "albergues", currentAlbergueId, "personas", prefiliacionEdicionId), { familiaId: fid, rolFamilia: 'TITULAR' }); }
            for (const f of adminFamiliaresTemp) { const ref = await addDoc(collection(db, "albergues", currentAlbergueId, "personas"), { ...f, estado: 'espera', familiaId: fid, rolFamilia: 'MIEMBRO', fechaRegistro: new Date() }); registrarLog(ref.id, "Registro Familiar", "Manual"); }
        }
        alert("Actualizado"); cancelarEdicionPref(); return;
    }
    const n = safeVal('man-nombre'); if (!n) return alert("Falta nombre"); const fid = new Date().getTime().toString(); const t = getDatosFormulario('man'); t.estado = 'espera'; t.familiaId = fid; t.rolFamilia = 'TITULAR'; t.fechaRegistro = new Date(); const ref = await addDoc(collection(db, "albergues", currentAlbergueId, "personas"), t); registrarLog(ref.id, "Registro Manual", "Titular");
    for (const f of adminFamiliaresTemp) { const refF = await addDoc(collection(db, "albergues", currentAlbergueId, "personas"), { ...f, estado: 'espera', familiaId: fid, rolFamilia: 'MIEMBRO', fechaRegistro: new Date() }); registrarLog(refF.id, "Registro Manual", "Familiar"); }
    alert("Guardado"); limpiarFormulario('man'); adminFamiliaresTemp = []; document.getElementById('admin-lista-familiares-ui').innerHTML = "Ninguno.";
}
...
function cerrarMapaCamas() { highlightedFamilyId = null; document.getElementById('modal-cama').classList.add('hidden'); }
function highlightFamily(pid) { const o = listaPersonasCache.find(p => p.id === pid); if (!o || !o.familiaId) return; highlightedFamilyId = (highlightedFamilyId === o.familiaId) ? null : o.familiaId; mostrarGridCamas(); }
async function guardarCama(c) { if (personaEnGestion.cama) { alert(`Error: Ya tiene cama.`); return; } await updateDoc(doc(db, "albergues", currentAlbergueId, "personas", personaEnGestion.id), { estado: 'ingresado', cama: c.toString(), fechaIngreso: new Date() }); registrarLog(personaEnGestion.id, "Asignación Cama", `Cama ${c}`); cerrarMapaCamas(); }
async function liberarCamaMantener() { await updateDoc(doc(db, "albergues", currentAlbergueId, "personas", personaEnGestion.id), { cama: null }); }
async function regresarPrefiliacion() { await updateDoc(doc(db, "albergues", currentAlbergueId, "personas", personaEnGestion.id), { estado: 'espera', cama: null }); }

async function abrirModalAlbergue(id = null) {
    albergueEdicionId = id; document.getElementById('modal-albergue').classList.remove('hidden'); const b = document.getElementById('btn-delete-albergue');
    if (id) {
    ...
    } else { setVal('mto-nombre', ""); setVal('mto-capacidad', ""); b.classList.add('hidden'); }
}
async function guardarAlbergue() {
    const n = safeVal('mto-nombre'), c = safeVal('mto-capacidad'), col = safeVal('mto-columnas'); if (!n || !c) return alert("Datos inc.");
...
async function eliminarAlbergueActual() {
    if (albergueEdicionId && confirm("¿Borrar todo?")) {
        const ps = await getDocs(collection(db, "albergues", albergueEdicionId, "personas")); const b = writeBatch(db); ps.forEach(d => b.delete(d.ref)); await b.commit();
        await deleteDoc(doc(db, "albergues", albergueEdicionId)); alert("Borrado"); document.getElementById('modal-albergue').classList.add('hidden');
    }
}
async function cambiarEstadoAlbergue(id, st) { await updateDoc(doc(db, "albergues", id), { activo: st }); }

function abrirModalCambioPass() { setVal('chg-old-pass', ''); setVal('chg-new-pass', ''); setVal('chg-confirm-pass', ''); document.getElementById('modal-change-pass').classList.remove('hidden'); }
async function ejecutarCambioPass() {
    const o = safeVal('chg-old-pass'), n = safeVal('chg-new-pass'), c = safeVal('chg-confirm-pass'); if (!o || !n || !c) return alert("Rellena todo"); if (n !== c) return alert("No coinciden"); if (n.length < 6) return alert("Min 6 chars");
    try { const u = auth.currentUser; await reauthenticateWithCredential(u, EmailAuthProvider.credential(u.email, o)); await updatePassword(u, n); alert("OK. Relogin"); document.getElementById('modal-change-pass').classList.add('hidden'); cerrarSesion(); } catch (e) { alert("Error: " + e.message); }
}

function cargarUsuarios() {
    const c = document.getElementById('lista-usuarios-container'); const filterText = safeVal('search-user').toLowerCase().trim();
    unsubscribeUsers = onSnapshot(query(collection(db, "usuarios")), s => {
        c.innerHTML = ""; if (s.empty) { c.innerHTML = "<p>No hay usuarios.</p>"; return; }
        s.forEach(d => {
            const u = d.data(); if (filterText && !u.nombre.toLowerCase().includes(filterText) && !u.email.toLowerCase().includes(filterText)) return;
            if (currentUserData.rol === 'admin' && u.rol === 'super_admin') return;
            c.innerHTML += `<div class="user-card-item" onclick="window.abrirModalUsuario('${d.id}')"><div class="user-card-left"><div class="user-avatar-circle">${u.nombre.charAt(0).toUpperCase()}</div><div><strong>${u.nombre}</strong><br><small>${u.email}</small></div></div><span class="badge role-${u.rol}">${u.rol}</span></div>`;
        });
    });
}
function filtrarUsuarios() { cargarUsuarios(); }
async function abrirModalUsuario(id = null) {
    userEditingId = id; document.getElementById('modal-crear-usuario').classList.remove('hidden'); const sel = document.getElementById('new-user-role'); sel.innerHTML = ""; const btnDel = document.getElementById('btn-delete-user');
    let roles = []; if (currentUserData.rol === 'super_admin') roles = ['super_admin', 'admin', 'intervencion', 'filiacion', 'observador']; else if (currentUserData.rol === 'admin') roles = ['intervencion', 'filiacion', 'observador'];
    roles.forEach(r => sel.add(new Option(r, r)));
    if (id) {
        const s = await getDoc(doc(db, "usuarios", String(id))); if (s.exists()) { const d = s.data(); setVal('new-user-name', d.nombre); setVal('new-user-email', d.email); sel.value = d.rol; if (['super_admin', 'admin'].includes(currentUserData.rol)) btnDel.classList.remove('hidden'); else btnDel.classList.add('hidden'); }
    } else { setVal('new-user-name', ""); setVal('new-user-email', ""); btnDel.classList.add('hidden'); }
}
async function guardarUsuario() {
    const e = safeVal('new-user-email'), p = safeVal('new-user-pass'), n = safeVal('new-user-name'), r = safeVal('new-user-role'); if (!n || !r) return alert("Datos incompletos");
    if (userEditingId) { await updateDoc(doc(db, "usuarios", userEditingId), { nombre: n, rol: r }); alert("Actualizado"); } else {
        if (!e || !p) return alert("Email y Pass requeridos"); let tApp;
        try { tApp = initializeApp(firebaseConfig, "Temp"); const tAuth = getAuth(tApp); const uc = await createUserWithEmailAndPassword(tAuth, e, p); await setDoc(doc(db, "usuarios", uc.user.uid), { email: e, nombre: n, rol: r }); await signOut(tAuth); alert("Creado"); } catch (err) { alert("Error: " + err.message); } finally { if (tApp) deleteApp(tApp); }
    }
    document.getElementById('modal-crear-usuario').classList.add('hidden');
}
async function eliminarUsuario() { if (!userEditingId || !confirm("¿Eliminar usuario?")) return; try { await deleteDoc(doc(db, "usuarios", userEditingId)); alert("Eliminado"); document.getElementById('modal-crear-usuario').classList.add('hidden'); } catch (e) { alert(e.message); } }

function abrirModalQR() {
    document.getElementById('modal-qr').classList.remove('hidden'); const qrDiv = document.getElementById("qrcode-display"); qrDiv.innerHTML = "";
    const url = window.location.href.split('?')[0] + `?public_id=${currentAlbergueId}`; new QRCode(qrDiv, { text: url, width: 250, height: 250 });
}
...
function mostrarGridCamas() {
    ...
    for (let i = 1; i <= totalCapacidad; i++) {
        ...
        d.onclick = () => { if (occ && !modoMapaGeneral) { abrirModalInfoCama(occ); } else if (!occ && !modoMapaGeneral) { guardarCama(n); } };
        d.ondblclick = () => { if (occ) abrirModalInfoCama(occ); };
        g.appendChild(d);
    }
    document.getElementById('modal-cama').classList.remove('hidden');
}
function abrirModalInfoCama(p) { document.getElementById('info-cama-num').innerText = p.cama; document.getElementById('info-nombre-completo').innerText = p.nombre; document.getElementById('modal-bed-info').classList.remove('hidden'); }

// --- INIT MODO PÚBLICO O PRIVADO ---
window.onload = () => {
...
// --- EXPORTAR A WINDOW AL FINAL (SEGURIDAD TOTAL) ---
...
window.regresarPrefiliacion = regresarPrefiliacion;
