...
let alberguesMap = {}; // Cache para nombres de albergues en notificaciones
...
let listaNotificacionesCache = []; // Cache global de notificaciones
...
window.getDatosFormulario = function(p) { 
    return { 
        ...
        tipoDoc: window.safeVal(`${p}-tipo-doc`), 
        docNum: window.safeVal(`${p}-doc-num`), 
        fechaNac: window.safeVal(`${p}-fecha`), 
...
window.sysLog = function(msg, type = 'info') {
    ...
    // DETECTOR DE FALTA DE ÍNDICE
    if (msg && (msg.includes("requires an index") || msg.includes("failed: preconf"))) {
        const urlMatch = msg.match(/https:\/\/[^\s]+/);
        const url = urlMatch ? urlMatch[0] : "https://console.firebase.google.com";
        div.innerHTML = `<span class="log-time">[${time}]</span> <strong style="color:red; background:white; padding:2px;">[FALTA ÍNDICE]</strong> <a href="${url}" target="_blank" style="color:yellow; font-weight:bold; text-decoration:underline;">HAGA CLIC AQUÍ PARA CREARLO</a>`;
...
window.onerror = function(message, source, lineno, colno, error) { 
    ...
    if(currentUserData && currentUserData.rol === 'super_admin') { 
        document.getElementById('black-box-overlay').classList.remove('hidden'); 
    } 
};
...
window.sysLog("Sistema Iniciado. Versión 2.9.1 (FULL CODE)", "info");
...
window.iniciarSesion = async function() { 
    try { 
        window.sysLog("Click Login", "info"); 
...
window.guardarUsuario = async function() { 
    ...
    try { 
        ...
        console.error(err); 
        if (err.code === 'auth/email-already-in-use') alert("ERROR: Correo ya registrado."); else alert("Error: " + err.message); 
    } 
};
...
window.desactivarUsuariosMasivo = async function() { 
    ...
    if (!confirm("⚠️ ATENCIÓN ⚠️\n\nEsta acción desactivará a TODOS los usuarios operativos.")) return; 
    ...
    try { 
        ...
        querySnapshot.forEach((doc) => { 
            ...
            if (u.rol !== 'super_admin' && u.rol !== 'admin') { 
                if (u.activo !== false) { batch.update(doc.ref, { activo: false }); count++; } 
            } 
        }); 
        if (count > 0) { 
            await batch.commit(); 
            window.sysLog(`Desactivados: ${count}`, "warn"); 
            alert(`Se han desactivado ${count} usuarios.`); 
        } else { alert("No había usuarios para desactivar."); } 
    } catch (e) { console.error(e); alert("Error: " + e.message); } 
    finally { window.safeHide('loading-overlay'); } 
};
...
window.abrirModalCambioPass = function() { 
    window.setVal('chg-old-pass',''); 
    window.setVal('chg-new-pass',''); 
    window.setVal('chg-confirm-pass',''); 
    window.safeShow('modal-change-pass'); 
};

window.ejecutarCambioPass = async function() { 
    const o = window.safeVal('chg-old-pass'), n = window.safeVal('chg-new-pass'); 
    try { 
        await reauthenticateWithCredential(auth.currentUser, EmailAuthProvider.credential(auth.currentUser.email, o)); 
        await updatePassword(auth.currentUser, n); 
        alert("OK"); 
        window.safeHide('modal-change-pass'); 
        window.sysLog("Contraseña cambiada.", "success"); 
    } catch(e) { 
        alert("Error"); 
        window.sysLog("Error cambio pass: "+e.message, "error"); 
    } 
};
...
window.navegar = function(p) {
    ...
    // NOTA: NO desconectamos las notificaciones globales aquí.

    // Si salimos al menú principal, reseteamos el contexto del albergue
        ...
        Object.keys(resumen).forEach(albId => {
            ...
            c.innerHTML += `<div class="notif-item" onclick="window.cargarDatosYEntrar('${albId}')"> 
                <div style="display:flex; justify-content:space-between; width:100%; align-items:center;">
                    ...
                    <span class="badge badge-active" style="background:var(--danger);">${num} Pendientes</span> 
                </div>
            </div>`;
        });
    }
...
window.verHistorial = async function(pId = null, forceIsGlobal = null, forceAlbId = null) {
    ...
    try {
        ...
        snapHist.forEach(d => { const data = d.data(); items.push({ ...data, type: 'movimiento', id: d.id, sortDate: data.fecha.toDate() }); });
        ...
        if (!isPool) {
            ...
            snapInt.forEach(d => {
                const data = d.data();
                items.push({ usuario: data.usuario, accion: data.tipo + ": " + data.subtipo, detalle: data.detalle, fecha: data.fecha, type: 'intervencion', rawType: data.tipo, id: d.id, sortDate: data.fecha.toDate() });
            });
        }
        ...
        items.forEach(d => {
            ...
            let extraClass = '';
            if (d.type === 'intervencion') {
                if (d.rawType === 'Sanitaria') extraClass = 'hist-type-san';
                else if (d.rawType === 'Psicosocial') extraClass = 'hist-type-psi';
                else if (d.rawType === 'Entregas') extraClass = 'hist-type-ent';
            }
...
window.cambiarAlberguePorQR = async function(aid, pid) { 
    window.sysLog(`Cambiando albergue por QR a: ${aid}`, "warn"); currentAlbergueId = aid; window.safeShow('loading-overlay'); 
    try { 
        const dS = await getDoc(doc(db,"albergues",aid)); 
        if(dS.exists()) { currentAlbergueData = dS.data(); totalCapacidad = parseInt(currentAlbergueData.capacidad||0); } 
        else { alert("Albergue no existe"); window.safeHide('loading-overlay'); return; } 
        if(unsubscribePersonas) unsubscribePersonas(); 
        unsubscribePersonas = onSnapshot(collection(db,"albergues",aid,"personas"), s=>{ listaPersonasCache=[]; camasOcupadas={}; s.forEach(d=>{ const p=d.data(); p.id=d.id; listaPersonasCache.push(p); if(p.estado==='ingresado'){ if(p.cama) camasOcupadas[p.cama]=p.nombre; } }); const target = listaPersonasCache.find(p => p.id === pid); if(target) { window.safeHide('loading-overlay'); window.navegar('intervencion'); window.cargarInterfazIntervencion(target); } }); 
        window.conectarListenersBackground(aid); 
        // Ya no suscribimos aquí porque es global desde el inicio
