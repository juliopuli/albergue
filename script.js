...
// --- 1. SISTEMA DE LOGS (ASIGNACIÓN DIRECTA A WINDOW) ---
...
window.toggleCajaNegra = function() {
    const bb = document.getElementById('black-box-overlay');
    if (bb) { if (bb.classList.contains('hidden')) { bb.classList.remove('hidden'); window.sysLog("Debug visual activado", "info"); } else { bb.classList.add('hidden'); } }
};
window.limpiarCajaNegra = function() { const c = document.getElementById('black-box-content'); if (c) c.innerHTML = ""; };

window.sysLog("Sistema Iniciado. Versión 38.7.0 (Robust + Dashboard)", "info");
...
// --- MODIFICADO: GESTIÓN CON CONTADORES ---
...
// --- MODIFICADO: DASHBOARD OBSERVATORIO ---
...
window.rescatarDeGlobalDirecto = async function() {
    ...
    try {
        ...
        await batch.commit(); window.sysLog(`Familia ingresada.`, "success"); window.showToast("Ingreso realizado. Asigne cama.");
        window.personaEnGestion = null; window.safeHide('panel-gestion-persona'); window.el('buscador-persona').value = "";
...
window.darSalidaPersona = async function() {
    ...
    try {
        ...
        await batch.commit(); window.sysLog(`Salida realizada.`, "nav"); window.showToast("Salida completada.");
...
window.liberarCamaMantener=async function(){await updateDoc(doc(db,"albergues",currentAlbergueId,"personas",personaEnGestion.id),{cama:null});window.sysLog("Cama liberada (Mantener)","warn");};
