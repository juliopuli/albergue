...
window.sysLog("Sistema Iniciado. V.5.0.3 (Nomenclatura Pre-Filiación)", "info");
...
window.eliminarPersonaGlobal = async function () {
    ...
    try {
        ...
        // Opcional: registrar en historial general del sistema (si existiera colección de logs globales)
        // pero por ahora solo borramos.

        await batch.commit();

        window.sysLog(`Persona eliminada de pre-filiación: ${personaEnGestion.nombre} por ${currentUserData.nombre}`, "warn");
        window.showToast("Persona eliminada correctamente");

        // Limpiar UI
        window.safeHide('panel-gestion-persona');
        window.safeHide('resultados-busqueda');
        window.el('buscador-persona').value = "";
        window.el('buscador-pref').value = ""; // Limpiar también buscador de pre-filiación

        // Actualizar lista global en memoria
...
window.mostrarGridCamas = function () {
    ...
    // 1. Agrupar familias para calcular "sombras" (reservas implícitas)
...
// [BLOQUE DUPLICADO ELIMINADO - guardarCama ya está definida correctamente arriba]
...
// Clean phantom derivations
window.limpiarDerivaciones = async function () {
    if (!confirm("¿Estás seguro de que deseas marcar TODAS las derivaciones pendientes como completadas? Esto arreglará el contador.")) return;
    ...
    try {
            ...
            for (const persDoc of personasSnap.docs) {
                ...
                for (const histDoc of historialSnap.docs) {
                    ...
                    if (log.estado === 'pendiente') {
                        await updateDoc(doc(db, "albergues", albDoc.id, "personas", persDoc.id, "historial", histDoc.id), {
                            estado: 'completado',
                            detalle: 'Limpieza automática de sistema (Reset)',
        ...
        console.error(e);
        window.sysLog("Error limpieza: " + e.message, "error");
        alert("Error durante la limpieza. Revisa el log.");
    }
};
...
let personaPrefiliacionPendiente = null;
let personasDuplicadasEncontradas = [];
let ignorarDuplicadoUnaVez = false;

window.detectarDuplicados = function (docNum, excluirId = null) {
    if (!docNum || docNum.trim() === '') return [];

    const docNumLimpio = docNum.trim().toUpperCase();

    return listaPersonasCache.filter(p => {
        if (!p.docNum) return false;
        if (p.estado === 'salida') return false;
        if (excluirId && p.id === excluirId) return false;

        return p.docNum.trim().toUpperCase() === docNumLimpio;
    });
};

window.mostrarAlertaDuplicados = function (personaPrefiliacion, duplicados) {
    personaPrefiliacionPendiente = personaPrefiliacion;
    personasDuplicadasEncontradas = duplicados;

    const infoDiv = document.getElementById('duplicados-info');

    let html = `
        <div style="margin-bottom: 15px; padding: 12px; background: #e7f3ff; border-radius: 8px;">
            <strong>Persona de Pre-Filiación:</strong><br>
            ${personaPrefiliacion.nombre} ${personaPrefiliacion.ap1 || ''}<br>
            <small>📄 ${personaPrefiliacion.docNum}</small>
        </div>
        
        <div style="margin-bottom: 10px;">
            <strong>Persona(s) duplicada(s) en el albergue:</strong>
        </div>
    `;

    duplicados.forEach(dup => {
        const estadoColor = dup.estado === 'ingresado' ? 'var(--success)' : 'var(--warning)';
        const estadoTexto = dup.estado === 'ingresado' ? 'INGRESADO' : dup.estado.toUpperCase();
        const camaInfo = dup.cama ? `🛏 Cama: ${dup.cama}` : '';

        html += `
            <div style="margin-bottom: 10px; padding: 12px; background: #fff; border: 2px solid #dee2e6; border-radius: 8px;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        <strong>${dup.nombre} ${dup.ap1 || ''} ${dup.ap2 || ''}</strong><br>
                        <small>📄 ${dup.docNum} ${camaInfo}</small>
                    </div>
                    <div style="padding: 4px 12px; background: ${estadoColor}; color: white; border-radius: 12px; font-size: 0.8rem;">
                        ${estadoTexto}
                    </div>
                </div>
            </div>
...
window.corregirDocumentoDuplicado = function () {
    ...
    setTimeout(() => {
        ...
        if (docInput) {
            docInput.focus();
            docInput.select();
            window.showToast('Corrige el número de documento y guarda los cambios');
        }
    }, 300);
};
