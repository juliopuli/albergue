...
window.sysLog = function(msg, type = 'info') {
    ...
    if (msg && (msg.includes("requires an index") || msg.includes("failed: preconf"))) {
        ...
        div.innerHTML = `<span class="log-time">[${time}]</span> <strong style="color:red; background:white; padding:2px;">[FALTA ÍNDICE]</strong> <a href="${url}" target="_blank" style="color:yellow; font-weight:bold; text-decoration:underline;">CREAR AQUÍ</a>`;
        const bb = document.getElementById('black-box-overlay'); if(bb) bb.classList.remove('hidden');
    } else {
...
onAuthStateChanged(auth, async (u) => {
    ...
    if(u){
        ...
        if(s.exists()){
            const d = s.data();
            if (d.activo === false) { alert("Usuario desactivado."); signOut(auth); return; }
            currentUserData = {...d, uid: u.uid};
            window.sysLog(`Usuario: ${currentUserData.nombre} (${currentUserData.rol})`, "success");
...
// ==========================================
// 3. NAVEGACIÓN Y DASHBOARD (PRIORIDAD ALTA)
// ==========================================
window.resetIntervencion = function() { 
    personaEnGestion = null; 
    window.safeHide('view-scan-result'); 
    window.safeShow('view-scan-ready'); 
    if(typeof resetScannerUI === 'function') resetScannerUI(); // Safe call
};

window.detenerEscaner = function() { 
    if (html5QrCode && html5QrCode.isScanning) { 
        html5QrCode.stop().then(() => { html5QrCode.clear(); }).catch(e=>console.log(e)).finally(() => { 
...
// --- NOTIFICACIONES (Lógica Simplificada) ---
window.suscribirNotificaciones = function(albId) {
    if(unsubscribeNotifs) unsubscribeNotifs();
    // Consulta SIN filtro de estado para evitar líos de índices, filtramos en JS
    let q = query(collection(db, "derivaciones_pendientes"), where("albergueId", "==", albId));
    window.sysLog(`Escuchando notificaciones...`, "info");
    ...
    unsubscribeNotifs = onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach(doc => {
            const data = doc.data();
            if(data.estado !== 'pendiente') return; // Filtro manual
            let show = false;
        ...
        if (count > 0) { window.safeShow('notification-badge-container'); countEl.innerText = count > 9 ? '9+' : count; badge.classList.add('shake-animation'); } 
        else { window.safeHide('notification-badge-container'); }
    }, (error) => { window.sysLog(`Error Notif: ${error.message}`, "error"); });
};
window.mostrarListaNotificaciones = function() {
    ...
    listaNotificacionesCache.forEach(n => {
        ...
        if(n.tipo === 'Sanitaria') tabDestino = 'sanitaria'; if(n.tipo === 'Psicosocial') tabDestino = 'psicosocial'; if(n.tipo === 'Entregas') tabDestino = 'entregas';
...
window.atenderNotificacion = async function(notifId, personaId, tabDestino) {
    ...
    try { await updateDoc(doc(db, "derivaciones_pendientes", notifId), { estado: 'atendida' }); } catch(e) { console.error("Error notif update", e); }
    ...
    } else { alert("Persona no encontrada (quizás salió)."); }
};
window.confirmarDerivacion = async function() {
    ...
    if(personaEnGestion) {
        ...
        try { await addDoc(collection(db, "derivaciones_pendientes"), { albergueId: currentAlbergueId, personaId: personaEnGestion.id, personaNombre: `${personaEnGestion.nombre} ${personaEnGestion.ap1 || ''}`, tipo: tipoDerivacionActual, motivo: motivo, fecha: new Date(), estado: 'pendiente' }); } catch(e) { console.error("Error derivación", e); }
    }
