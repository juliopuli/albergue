...
window.onload=async()=>{
    ...
    if(p.get('public_id')){
        ...
        try{
            const s=await getDoc(doc(db,"albergues",currentAlbergueId));
            if(s.exists()) document.getElementById('public-albergue-name').innerText="Albergue "+s.data().nombre;
            else document.getElementById('public-albergue-name').innerText="este Albergue";
        }catch(e){document.getElementById('public-albergue-name').innerText="este Albergue";}
    }
};
...
window.entrarAlbergue=(id)=>{
    ...
    window.cambiarPestana('prefiliacion'); // RESET A PREFILIACION COMO V40
