...
window.verListaObservatorio = async (albId, est) => {
    ...
    try {
        ...
        // ORDENACIÓN (PUNTO 1 DE V4.5.0)
        let dataArray = [];
        s.forEach(doc => { dataArray.push({ id: doc.id, ...doc.data() }); });

        // Si son ingresados, ordenar por cama. Si no, dejar como está (o por fecha si se pidiera después)
        if (est === 'ingresado') {
            dataArray.sort((a, b) => (parseInt(a.cama)||0) - (parseInt(b.cama)||0));
        }

        // TABLA CON COLUMNA CAMA SI ES INGRESADO
        let h = `<table class="fam-table"><thead><tr><th style="width:40px;"></th>`;
        if(est === 'ingresado') h += `<th>Cama</th>`; // NUEVA COLUMNA
        h += `<th>Nombre</th><th>DNI</th><th>Tel</th></tr></thead><tbody>`;
        
        dataArray.forEach(d => { 
            h += `<tr>
                <td style="text-align:center;"><button class="btn-icon-small" onclick="window.verHistorialObservatorio('${albId}', '${d.id}')"><i class="fa-solid fa-clock-rotate-left"></i></button></td>`;
...
window.liberarCamaMantener=async()=>{ const oldCama = window.personaEnGestion.cama; await updateDoc(doc(db,"albergues",currentAlbergueId,"personas",window.personaEnGestion.id),{cama:null}); window.registrarLog(window.personaEnGestion.id, "Liberar Cama", `Liberada cama ${oldCama}`); };
window.regresarPrefiliacion=async()=>{ const oldCama = window.personaEnGestion.cama; await updateDoc(doc(db,"albergues",currentAlbergueId,"personas",window.personaEnGestion.id),{estado:'espera',cama:null}); window.registrarLog(window.personaEnGestion.id, "Retorno Pre-filiación", `Liberada cama ${oldCama}`); };
