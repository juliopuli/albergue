...
window.navegar=(p)=>{
    ...
    if(p==='gestion-albergues' || p==='operativa') document.getElementById('nav-albergues').classList.add('active');
    if(p==='mantenimiento') document.getElementById('nav-mto').classList.add('active');
};
...
window.cambiarPestana = (t) => {
    ...
    if (t === 'prefiliacion') {
        ...
        document.getElementById('resultados-busqueda').classList.add('hidden'); // Use class, not style
        document.getElementById('panel-gestion-persona').classList.add('hidden');
    }
};
...
// --- BUSCADORES Y FILIACIÓN (FULL REPAIR) ---
...
window.buscarPersonaEnAlbergue=()=>{
    ...
    if(hits.length===0){
        ...
        hits.forEach(p=>{
            // DOT LOGIC
            const dotClass = p.cama ? 'dot-green' : 'dot-orange';
