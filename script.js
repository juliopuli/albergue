...
function actualizarContadores() { const elOcc = el('ocupacion-count'); const elCap = el('capacidad-total'); if (elOcc) elOcc.innerText = ocupacionActual; if (elCap) elCap.innerText = totalCapacidad; }
function showToast(msg) { const t = el('toast'); if(t) { t.innerText = msg; t.classList.add('show'); setTimeout(() => t.classList.remove('show'), 2000); } }
...
// --- 3. LOGICA PRINCIPAL (DECLARACIONES) ---
async function iniciarSesion() { try { await signInWithEmailAndPassword(auth, el('login-email').value, el('login-pass').value); sysLog("Login OK", "success"); } catch(e) { sysLog(e.message, "error"); alert(e.message); } }
...
function navegar(p) {
    ...
    if(p.includes('albergue')) safeAddActive('nav-albergues'); else if(p.includes('obs')) safeAddActive('nav-obs'); else if(p.includes('mantenimiento')) safeAddActive('nav-mto'); else safeAddActive('nav-home');
}
...
function configurarTabsPorRol() { const r = (currentUserData.rol || "").toLowerCase().trim(); ['btn-tab-pref', 'btn-tab-fil', 'btn-tab-san', 'btn-tab-psi'].forEach(id => safeShow(id)); if (r === 'intervencion') { safeHide('btn-tab-pref'); safeHide('btn-tab-fil'); return 'sanitaria'; } return 'filiacion'; }
function cambiarPestana(t) { ['tab-prefiliacion', 'tab-filiacion', 'tab-sanitaria', 'tab-psicosocial'].forEach(id => safeHide(id)); ['btn-tab-pref', 'btn-tab-fil', 'btn-tab-san', 'btn-tab-psi'].forEach(id => safeRemoveActive(id)); safeAddActive(`btn-tab-${t.substring(0,3)}`); safeShow(`tab-${t}`); if (t === 'prefiliacion') { limpiarFormulario('man'); adminFamiliaresTemp = []; actualizarListaFamiliaresAdminUI(); if(el('existing-family-list-ui')) el('existing-family-list-ui').innerHTML = ""; cancelarEdicionPref(); setupAutoSave(); } else if (t === 'filiacion') { if(el('buscador-persona')) el('buscador-persona').value = ""; safeHide('resultados-busqueda'); safeHide('panel-gestion-persona'); personaEnGestion = null; } }

// --- DATA CORE ---
async function cargarDatosYEntrar(id) {
    currentAlbergueId = id; safeShow('loading-overlay');
    try {
        ...
        unsubscribePool = onSnapshot(collection(db, "pool_prefiliacion"), s => { listaGlobalPrefiliacion = []; s.forEach(d => { const p = d.data(); p.id = d.id; listaGlobalPrefiliacion.push(p); }); });
        navegar('operativa');
        if(el('app-title')) el('app-title').innerText = currentAlbergueData.nombre;
        configurarDashboard(); actualizarContadores(); safeHide('loading-overlay'); conectarListenersBackground(id); setupAutoSave();
    } catch(e) { sysLog(e.message, "error"); alert(e.message); safeHide('loading-overlay'); }
}
...
// --- OBSERVATORIO (MODIFICADO) ---
async function cargarObservatorio() {
    const list = el('obs-list-container'); 
    if(!list) return;
    list.innerHTML = '<div style="text-align:center; padding:20px;"><div class="spinner"></div></div>';
    el('kpi-espera').innerText = "-"; el('kpi-alojados').innerText = "-";
    el('kpi-libres').innerText = "-"; el('kpi-percent').innerText = "-%";
    ...
    try {
        ...
        const promesas = alberguesSnap.docs.map(async (docAlb) => {
            ...
            const personasSnap = await getDocs(collection(db, "albergues", docAlb.id, "personas"));
            let w = 0, h = 0;
            personasSnap.forEach(p => { const pd = p.data(); if(pd.estado === 'espera') w++; if(pd.estado === 'ingresado') h++; });
            return { nombre: dataAlb.nombre, capacidad: cap, espera: w, alojados: h };
        });
        ...
        resultados.forEach(res => {
            ...
            htmlList += `<div class="obs-row"><div class="obs-row-title">${res.nombre}</div><div class="obs-stats-group"><div class="obs-mini-stat"><span>Espera</span><strong>${res.espera}</strong></div><div class="obs-mini-stat"><span>Alojados</span><strong>${res.alojados}</strong></div><div class="obs-mini-stat"><span>Libres</span><strong>${libres}</strong></div></div><div class="prog-container"><div style="display:flex; justify-content:space-between; font-size:0.8rem; margin-bottom:2px;"><span>Ocupación</span><strong>${porcentaje}%</strong></div><div class="prog-track"><div class="prog-fill ${barClass}" style="width: ${porcentaje}%"></div></div></div></div>`;
        });
        ...
        el('kpi-espera').innerText = totalEspera; el('kpi-alojados').innerText = totalAlojados;
        el('kpi-libres').innerText = globalLibres; el('kpi-percent').innerText = `${globalPercent}%`;
        list.innerHTML = htmlList;
    } catch(e) { sysLog("Error obs: " + e.message, "error"); list.innerHTML = "<p>Error cargando datos.</p>"; }
}
...
async function guardarCambiosPersona(silent=false) { if(!personaEnGestion)return; const p=getDatosFormulario('edit'); await updateDoc(doc(db,"albergues",currentAlbergueId,"personas",personaEnGestion.id),p); registrarLog(personaEnGestion.id,"Edición Datos","Manual"); if(!silent) alert("Guardado"); else showToast("Guardado automático"); }
async function rescatarDeGlobalDirecto() { if(!personaEnGestion || !personaEnGestionEsGlobal) return; if(!confirm(`¿Ingresar a ${personaEnGestion.nombre}?`)) return; try { const familia = listaGlobalPrefiliacion.filter(x => x.familiaId === personaEnGestion.familiaId); const batch = writeBatch(db); familia.forEach(member => { const localRef = doc(collection(db, "albergues", currentAlbergueId, "personas")); const memberData = {...member}; delete memberData.id; memberData.fechaIngresoAlbergue = new Date(); memberData.origenPoolId = member.id; memberData.estado = 'espera'; batch.set(localRef, memberData); batch.delete(doc(db, "pool_prefiliacion", member.id)); batch.set(doc(collection(db, "albergues", currentAlbergueId, "personas", localRef.id, "historial")), {fecha: new Date(), usuario: currentUserData.nombre, accion: "Ingreso desde Nube", detalle: "Rescatado"}); }); await batch.commit(); showToast("Ingreso realizado."); personaEnGestion = null; safeHide('panel-gestion-persona'); el('buscador-persona').value = ""; } catch(e) { sysLog(e.message, "error"); } }
async function darSalidaPersona() { if(!personaEnGestion || personaEnGestionEsGlobal) return; if(!confirm(`¿Dar salida?`)) return; try { const batch = writeBatch(db); const poolRef = doc(collection(db, "pool_prefiliacion")); const memberData = {...personaEnGestion}; delete memberData.id; memberData.cama = null; memberData.estado = 'espera'; memberData.fechaSalidaAlbergue = new Date(); memberData.ultimoAlbergueId = currentAlbergueId; batch.set(poolRef, memberData); batch.delete(doc(db, "albergues", currentAlbergueId, "personas", personaEnGestion.id)); await batch.commit(); showToast("Salida completada."); safeHide('panel-gestion-persona'); safeHide('resultados-busqueda'); el('buscador-persona').value = ""; } catch(e) { sysLog(e.message, "error"); } }
async function liberarCamaMantener() { if(!personaEnGestion) return; if(!confirm(`¿Liberar cama?`)) return; try { await updateDoc(doc(db, "albergues", currentAlbergueId, "personas", personaEnGestion.id), { cama: null }); sysLog("Cama liberada.", "success"); if(!modoMapaGeneral) cerrarMapaCamas(); } catch(e) { sysLog(e.message, "error"); } }
...
async function guardarCama(c) { if (personaEnGestionEsGlobal) { if(!confirm(`¿Ingresar y asignar cama ${c}?`)) return; try { const familia = listaGlobalPrefiliacion.filter(x => x.familiaId === personaEnGestion.familiaId); const batch = writeBatch(db); let newPersonLocalId = null; familia.forEach(member => { const localRef = doc(collection(db, "albergues", currentAlbergueId, "personas")); const memberData = {...member}; delete memberData.id; memberData.fechaIngresoAlbergue = new Date(); memberData.origenPoolId = member.id; if(member.id === personaEnGestion.id) { memberData.estado = 'ingresado'; memberData.cama = c.toString(); memberData.fechaIngreso = new Date(); newPersonLocalId = localRef.id; } else { memberData.estado = 'espera'; } batch.set(localRef, memberData); batch.delete(doc(db, "pool_prefiliacion", member.id)); }); await batch.commit(); cerrarMapaCamas(); showToast("Ingresado."); setTimeout(() => { const newPerson = listaPersonasCache.find(p => p.id === newPersonLocalId); if(newPerson) seleccionarPersona(newPerson, false); else { safeHide('panel-gestion-persona'); el('buscador-persona').value = ""; } }, 800); } catch(e) { sysLog(e.message, "error"); } return; } if(personaEnGestion.cama){ alert(`Error: Ya tiene cama.`); return; } await updateDoc(doc(db,"albergues",currentAlbergueId,"personas",personaEnGestion.id),{estado:'ingresado',cama:c.toString(),fechaIngreso:new Date()}); cerrarMapaCamas(); sysLog(`Cama ${c} asignada`, "success"); }
function mostrarGridCamas() { const g=el('grid-camas'); g.innerHTML=""; const cols=(currentAlbergueData&&currentAlbergueData.columnas)?currentAlbergueData.columnas:8; g.style.gridTemplateColumns=`repeat(${cols}, 1fr)`; let shadowMap={}; let famGroups={}; listaPersonasCache.forEach(p=>{if(p.familiaId){if(!famGroups[p.familiaId])famGroups[p.familiaId]={members:[],beds:[]};famGroups[p.familiaId].members.push(p);if(p.cama)famGroups[p.familiaId].beds.push(parseInt(p.cama));}}); Object.values(famGroups).forEach(fam=>{let assigned=fam.beds.length;let total=fam.members.length;let needed=total-assigned;if(assigned>0&&needed>0){let startBed=Math.max(...fam.beds);let placed=0;let check=startBed+1;while(placed<needed&&check<=totalCapacidad){if(!camasOcupadas[check.toString()]){shadowMap[check.toString()]=fam.members[0].familiaId;placed++;}check++;}}}); for(let i=1;i<=totalCapacidad;i++){ const n=i.toString(); const occName=camasOcupadas[n]; const occ=listaPersonasCache.find(p=>p.cama===n); let cls="bed-box"; let lbl=n; if(occ&&highlightedFamilyId&&occ.familiaId===highlightedFamilyId){cls+=" bed-family-highlight";} if(!modoMapaGeneral&&personaEnGestion&&personaEnGestion.cama===n){cls+=" bed-current";lbl+=" (Tú)";} else if(occName){ cls+=" bed-busy"; if(occ){const f=`${occ.nombre} ${occ.ap1||''}`;lbl+=`<div style="font-size:0.6rem;font-weight:normal;margin-top:2px;">${f}<br><i class="fa-solid fa-phone"></i> ${occ.telefono||'-'}</div>`;} } else{ cls+=" bed-free"; if(shadowMap[n]){cls+=" bed-shadow";} } const d=document.createElement('div'); d.className=cls; d.innerHTML=lbl; d.onclick=()=>{if(occ){if(highlightedFamilyId===occ.familiaId)highlightedFamilyId=null;else highlightedFamilyId=occ.familiaId;mostrarGridCamas();}else if(!modoMapaGeneral){guardarCama(n);}}; d.ondblclick=()=>{if(occ)abrirModalInfoCama(occ);}; g.appendChild(d); } safeShow('modal-cama'); }
function abrirModalInfoCama(p){ el('info-cama-num').innerText=p.cama; el('info-nombre-completo').innerText=p.nombre; const bh=el('btn-historial-cama'); if(['admin','super_admin'].includes(currentUserData.rol)){ safeShow('btn-historial-cama'); bh.onclick=()=>verHistorial(p.id); }else{ safeHide('btn-historial-cama'); } const c=el('info-familia-detalle'); const fam=listaPersonasCache.filter(x=>x.familiaId===p.familiaId); let h=`<table class="fam-table"><thead><tr><th>Nombre</th><th>Cama</th></tr></thead><tbody>`; fam.forEach(f=>{ const isCurrent=f.id===p.id?'fam-row-current':''; h+=`<tr class="${isCurrent}"><td>${f.nombre}</td><td><strong>${f.cama||'-'}</strong></td></tr>`; }); h+=`</tbody></table>`; c.innerHTML=h; safeShow('modal-bed-info'); }

function buscarEnPrefiliacion() { const t = safeVal('buscador-pref').toLowerCase().trim(); const r = el('resultados-pref'); if (t.length < 2) { safeHide('resultados-pref'); return; } const hits = listaGlobalPrefiliacion.filter(p => { const f = `${p.nombre} ${p.ap1||''} ${p.ap2||''}`.toLowerCase(); return f.includes(t) || (p.docNum||"").toLowerCase().includes(t); }); r.innerHTML = ""; if (hits.length === 0) r.innerHTML = "<div class='search-item'>Sin resultados</div>"; hits.forEach(p => { r.innerHTML += `<div class="search-item" onclick="window.exposed_cargarParaEdicionPref('${p.id}')"><strong>${p.nombre}</strong> (Nube)</div>`; }); safeShow('resultados-pref'); }
function cargarParaEdicionPref(pid) { const p = listaGlobalPrefiliacion.find(x => x.id === pid); if (!p) return; prefiliacionEdicionId = p.id; isGlobalEdit = true; safeHide('resultados-pref'); el('buscador-pref').value = ""; setVal('man-nombre', p.nombre); setVal('man-ap1', p.ap1); setVal('man-ap2', p.ap2); setVal('man-tipo-doc', p.tipoDoc); setVal('man-doc-num', p.docNum); setVal('man-fecha', p.fechaNac); setVal('man-tel', p.telefono); const l = el('existing-family-list-ui'); l.innerHTML = ""; if (p.familiaId) { const fs = listaGlobalPrefiliacion.filter(x => x.familiaId === p.familiaId && x.id !== p.id); if (fs.length > 0) { l.innerHTML = "<h5>Familiares:</h5>"; fs.forEach(f => { l.innerHTML += `<div>${f.nombre}</div>`; }); } } el('btn-save-pref').innerText = "Actualizar en Pool"; safeShow('btn-cancelar-edicion-pref'); safeShow('btn-ingresar-pref'); }
function cancelarEdicionPref() { prefiliacionEdicionId = null; limpiarFormulario('man'); if(el('existing-family-list-ui')) el('existing-family-list-ui').innerHTML=""; safeHide('btn-cancelar-edicion-pref'); safeHide('btn-ingresar-pref'); }
...
async function vincularAFamilia(targetId){ /* Simplified */ }
...
async function publicoGuardarTodo(){ const d=getDatosFormulario('pub'); if(!d.nombre)return; const fid=new Date().getTime().toString(); const b=writeBatch(db); const tRef=doc(collection(db,"pool_prefiliacion")); b.set(tRef, {...d, familiaId:fid, rolFamilia:'TITULAR', estado:'espera', origenAlbergueId:currentAlbergueId, fechaRegistro:new Date()}); listaFamiliaresTemp.forEach(f=>{ const fRef=doc(collection(db,"pool_prefiliacion")); b.set(fRef, {...f, familiaId:fid, rolFamilia:'MIEMBRO', estado:'espera', origenAlbergueId:currentAlbergueId, fechaRegistro:new Date()}); }); await b.commit(); safeHide('public-form-container'); safeShow('public-success-msg'); }
function abrirModalCambioPass(){ setVal('chg-old-pass',''); setVal('chg-new-pass',''); safeShow('modal-change-pass'); }
async function ejecutarCambioPass(){ safeHide('modal-change-pass'); }

// ============================================
// 4. EXPOSICIÓN A WINDOW
// ============================================
Object.assign(window, {
    ...
    buscarEnPrefiliacion, cargarParaEdicionPref, buscarPersonaEnAlbergue, seleccionarPersona, rescatarDeGlobalDirecto, darSalidaPersona, guardarCama, setupAutoSave, adminPrefiliarManual, registrarLog, verHistorial,
    ...
    cargarUsuarios, filtrarUsuarios, abrirModalUsuario, guardarUsuario, eliminarUsuario,
    publicoGuardarTodo, toggleStartButton, iniciarRegistro, cargarDatosYEntrar, liberarCamaMantener, cargarObservatorio, cambiarEstadoAlbergue
});
