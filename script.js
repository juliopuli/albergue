...
// --- 4. LÓGICA QR PÚBLICA (FIX HISTORIAL) ---
...
window.publicoGuardarTodo = async function() {
    ...
    try {
        ...
        // 1. COMMIT FIRST (Ensure person exists)
        await b.commit();
        
        // 2. LOG AFTER COMMIT (Safe write)
        window.registrarLog(tRef.id, "Auto-Registro QR", "Alta Titular (QR)");
        // Note: For family members we'd need their IDs, simplified to log titular for now or we iterate again, but usually Titular log is enough for the group entry.
