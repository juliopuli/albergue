...
import { getFirestore, collection, addDoc, setDoc, query, where, getDocs, doc, updateDoc, onSnapshot, deleteDoc, writeBatch } 
...
// Detectar modo público (QR de entrada)
...
let listaNotificacionesCache = []; // Cache global
...
window.onerror = function(message, source, lineno, colno, error) { 
    ...
    if(currentUserData && currentUserData.rol === 'super_admin') { document.getElementById('black-box-overlay').classList.remove('hidden'); } 
};

window.toggleCajaNegra = function() { const bb = document.getElementById('black-box-overlay'); if (bb) bb.classList.toggle('hidden'); };
window.limpiarCajaNegra = function() { const c = document.getElementById('black-box-content'); if (c) c.innerHTML = ""; };

// ==========================================
// 4. FUNCIONES DE SEGURIDAD UI (TOP PRIORITY)
// ==========================================
// Estas funciones se definen antes que nada para evitar errores al renderizar botones
...
window.verCarnetQR = function() { 
    ...
    const container = window.el('carnet-qrcode-display'); container.innerHTML = ""; 
    ...
    window.el('carnet-nombre').innerText = `${personaEnGestion.nombre} ${personaEnGestion.ap1 || ""}`; 
...
window.navegar = function(p) {
    ...
    // Limpieza de escuchadores locales
    if(unsubscribeUsers) unsubscribeUsers(); 
    if(unsubscribeAlberguesActivos) unsubscribeAlberguesActivos();
    // NOTA: NO desconectamos las notificaciones globales aquí (unsubscribeNotifs).

    // Si salimos al menú principal, reseteamos el contexto
...
window.cambiarPestana = function(t) { 
    ...
    if (t === 'prefiliacion') { 
        ...
        if(typeof window.cerrarFormularioIntervencion === 'function') window.cerrarFormularioIntervencion(prefix);
    }
};
...
window.configurarDashboard = function() { 
    ...
    const navItems = document.querySelectorAll('.nav-item'); navItems.forEach(n => n.classList.remove('active', 'disabled', 'hidden')); 
...
// ==========================================
// 7. GESTIÓN DE CAMAS (LÓGICA COMPLETA)
// ==========================================
...
window.mostrarGridCamas = function() {
    ...
    // Algoritmo de "Sombra" para sugerir camas contiguas a familias
    ...
    for (let i = 1; i <= totalCapacidad; i++) {
        ...
        if (!window.modoMapaGeneral && window.personaEnGestion && window.personaEnGestion.cama === n) {
            ...
            if (occ) {
                ...
                if (presencia === 'dentro') cls += " bed-status-in"; else cls += " bed-status-out";
            }
        } else {
            cls += " bed-free";
            if (shadowMap[n]) {
...
// ==========================================
// 8. NOTIFICACIONES GLOBALES (Versión v2.7.2)
// ==========================================
window.suscribirNotificacionesGlobales = async function() {
    ...
    } catch(e) { console.warn("Error mapa albergues", e); }
...
window.atenderNotificacion = async function(notifId, personaId, tabDestino) {
    ...
    setTimeout(() => {
        ...
        } else { alert("La persona no se encuentra en la lista activa de este albergue."); }
    }, 200);
};
...
// ==========================================
// 10. FUNCIONES RESTANTES (ADMIN, ETC)
// ==========================================
...
window.guardarCambiosPersona = async function() { 
    if(!personaEnGestion) return; 
    const p=window.getDatosFormulario('edit'); 
    await updateDoc(doc(db,"albergues",currentAlbergueId,"personas",personaEnGestion.id),p); 
    window.registrarLog(personaEnGestion.id,"Edición","Manual"); 
    window.showToast("Guardado"); 
};

window.darSalidaPersona = async function() { 
    if(!confirm("Dar salida?")) return; 
    const b=writeBatch(db); 
    const pr=doc(collection(db,"pool_prefiliacion")); 
    const md={...personaEnGestion}; delete md.id; md.cama=null; md.estado='espera'; md.fechaSalida=new Date(); 
    b.set(pr,md); b.delete(doc(db,"albergues",currentAlbergueId,"personas",personaEnGestion.id)); 
    await b.commit(); 
    window.showToast("Salida OK"); 
    window.safeHide('panel-gestion-persona'); 
};
...
window.cargarAlberguesMantenimiento = function() { const c = window.el('mto-container'); onSnapshot(query(collection(db,"albergues")), s=>{ c.innerHTML="<div class='mto-card add-new' onclick='window.abrirModalAlbergue()'><h3>+</h3></div>"; s.forEach(d=>{ const a=d.data(); c.innerHTML+=`<div class="mto-card"><h3>${a.nombre}</h3><button class="secondary" onclick="window.abrirModalAlbergue('${d.id}')">Edit</button></div>`; }); }); };
...
window.cargarUsuarios = function() { const c = window.el('lista-usuarios-container'); onSnapshot(collection(db,"usuarios"), s=>{ c.innerHTML=""; s.forEach(d=>{ const u=d.data(); c.innerHTML+=`<div class="user-card-item" onclick="window.abrirModalUsuario('${d.id}')"><strong>${u.nombre}</strong><br>${u.rol}</div>`; }); }); };
window.guardarUsuario = async function() { const e=window.el('new-user-email').value, p=window.el('new-user-pass').value, n=window.el('new-user-name').value, r=window.el('new-user-role').value; if(userEditingId) await updateDoc(doc(db,"usuarios",userEditingId),{nombre:n,rol:r}); else { const c = await createUserWithEmailAndPassword(auth,e,p); await setDoc(doc(db,"usuarios",c.user.uid),{email:e,nombre:n,rol:r,activo:true}); } window.safeHide('modal-crear-usuario'); };

window.iniciarEscanerReal = function() { window.detenerEscaner(); window.safeHide('scan-placeholder'); window.safeHide('btn-start-camera'); window.safeShow('reader'); window.safeShow('btn-stop-camera'); setTimeout(() => { if (!html5QrCode) html5QrCode = new Html5Qrcode("reader"); html5QrCode.start({ facingMode: "environment" }, { fps: 10 }, window.onScanSuccess).catch(e=>{}); }, 300); };
window.onScanSuccess = function(t) { if(html5QrCode) html5QrCode.stop().then(()=>{ const u=new URL(t); window.cambiarAlberguePorQR(u.searchParams.get("aid"), u.searchParams.get("pid")); }); };
window.cambiarAlberguePorQR = async function(aid, pid) { currentAlbergueId=aid; await window.cargarDatosYEntrar(aid); setTimeout(()=>{ const p=listaPersonasCache.find(x=>x.id===pid); if(p) { window.navegar('intervencion'); window.cargarInterfazIntervencion(p); } }, 1200); };
window.cargarInterfazIntervencion = function(p) { personaEnGestion=p; window.safeHide('view-scan-ready'); window.safeShow('view-scan-result'); window.el('interv-nombre').innerText=p.nombre; };
window.registrarMovimiento = async function(t) { if(!personaEnGestion) return; await updateDoc(doc(db,"albergues",currentAlbergueId,"personas",personaEnGestion.id),{presencia:t==='entrada'?'dentro':'fuera'}); window.showToast("Registrado"); window.resetIntervencion(); };
window.abrirModalDerivacion = function(t) { tipoDerivacionActual=t; window.el('derivacion-titulo').innerText=t; window.safeShow('modal-derivacion'); };
