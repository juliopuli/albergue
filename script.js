...
// --- HELPER FUNCIONES SEGURAS ---
// ESTA ES LA CLAVE PARA QUE NO SE ROMPA EL QR NI NADA
function getVal(id) {
    const el = document.getElementById(id);
    return el ? el.value : ""; // Devuelve vacío si no existe el input (ej. telefono en familia)
}
...
window.navegar=(p)=>{
    ...
    // Simple logic for active class (optional improvement)
};
...
window.validarEdad=(p)=>{const f=document.getElementById(`${p}-fecha`).value;if(f.length!==10)return true; return true;};
...
function getDatosFormulario(p) {
    return {
        ...
        ap1: getVal(`${p}-ap1`),
        ap2: getVal(`${p}-ap2`),
        tipoDoc: getVal(`${p}-tipo-doc`),
        docNum: getVal(`${p}-doc-num`),
        fechaNac: getVal(`${p}-fecha`),
        telefono: getVal(`${p}-tel`)
    };
}
...
window.guardarFamiliarEnLista=()=>{
    ...
    const d = getDatosFormulario('fam'); // Uses SAFE getVal
...
window.entrarAlbergue=(id)=>{currentAlbergueId=id;window.navegar('operativa');onSnapshot(doc(db,"albergues",id),d=>{currentAlbergueData=d.data();document.getElementById('app-title').innerText=currentAlbergueData.nombre;totalCapacidad=currentAlbergueData.capacidad||0;actualizarContadores();});unsubscribePersonas=onSnapshot(query(collection(db,"albergues",id,"personas")),s=>{listaPersonasCache=[];let c=0;camasOcupadas={};s.forEach(ds=>{const p=ds.data();p.id=ds.id;listaPersonasCache.push(p);if(p.estado!=='espera'){c++;if(p.cama)camasOcupadas[p.cama]=p.nombre;}});
    listaPersonasCache.sort((a,b)=>(b.fechaRegistro?.seconds||0)-(a.fechaRegistro?.seconds||0)); // LOCAL SORT
    ocupacionActual=c;actualizarContadores();if(window.personaEnGestion){const upd=listaPersonasCache.find(u=>u.id===window.personaEnGestion.id);if(upd)seleccionarPersona(upd);}});};
function actualizarContadores(){const elOcc=document.getElementById('ocupacion-count');const elCap=document.getElementById('capacidad-total');if(elOcc)elOcc.innerText=ocupacionActual;if(elCap)elCap.innerText=totalCapacidad;}
