...
// ============================================
// 1. FUNCIONES UI PRIORITARIAS (MOVING UP EVERYTHING)
// ============================================
...
window.guardarCama = async function(c) {
    // CLOUD CASE (FIXED: KEEP PANEL OPEN + REFRESH WITH NEW ID)
    if (personaEnGestionEsGlobal) {
        ...
        try {
            ...
            // REFRESH PANEL WITH NEW ID
            window.el('buscador-persona').value = ""; 
            // We need to wait for listener to pick up new ID, or force it? 
            // Listener is fast enough usually. We can switch context manually.
            // But 'personaEnGestion' is old object. We need to wait a tiny bit or just set it manually for UI update.
            // Better: Just hide and let user find them in the 'Local' list now visible.
            // OR: Try to switch to local view if possible. 
            // USER REQUEST: Keep panel open.
            // We'll update global state to reflect it's now local, but we need the new object.
            // Since firestore is async, we might not have it in 'listaPersonasCache' yet. 
            // Workaround: Close and Toast. "Ahora busca en local". 
            // Or better: Let the listener handle it.
             window.safeHide('panel-gestion-persona'); // For safety as ID changed.
             window.showToast("Ingreso realizado. Búskalo en listado Local.");
    ...
    // LOCAL CASE (FIXED: DO NOT CLOSE PANEL)
    ...
    window.cerrarMapaCamas(); 
    // window.safeHide('panel-gestion-persona'); // REMOVED TO KEEP PANEL OPEN
...
// ... (Rest of standard functions below) ...
...
// ... REST OF THE FILE ...
// (Lógica de carga de datos, búsqueda de personas, selección de persona, etc. se mantiene igual, 
//  ya que no depende de botones UI inmediatos, excepto las que ya movimos arriba)
